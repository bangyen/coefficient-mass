...
while holding in the second, and finitely many certificates on intervals
of roots show that it fails in the second row of some diagonal for every
$r\in[\frac{11}{10},\frac{139}{100}]$.
\end{abstract}
...
(Proposition~\ref{prop:secondrowhyp}).  Since the values
$\mu_r(S,L)$ decrease with $r$, a failure certified at two roots holds
between them, and finitely many certificates show that the identity fails
in the second row of some diagonal for every
$r\in[\frac{11}{10},\frac{139}{100}]$ (Theorem~\ref{thm:failinterval}).
Every row $k\ge2$ is likewise the minimum of $V_r(L-1,k-1)$ and finitely many
...
third row of a diagonal while holding in the second.  The proofs of these
three propositions and of Theorem~\ref{thm:failinterval} are
computer-assisted: they list the certifying zero sets and
...
when $\nu_2(n)>\nu_1(n)$, and fails somewhere in the second row at every
root of an interval, and
...
and $37$ of them show that the identity fails in the second row of some
diagonal at every $r\in[\frac{11}{10},\frac{139}{100}]$
...
decreases with $r$, a failure certified at two roots persists between
them (Proposition~\ref{prop:failinterval}), and finitely many such
...
$\eta_\sigma(n;r)>\max\{\nu_1(n;r),\nu_2(n;r)\}$, and
Proposition~\ref{prop:rowvalue}(b) with $S=\{\sigma\}$ gives
...
For every $r\in[\frac{11}{10},\frac{139}{100}]$ there is an $n$ with
$10\le n\le36$ such that $\eta_2(n)>\max\{\nu_1(n),\nu_2(n)\}$ at the root
...
lists $37$ tuples $(r_0,r_1,n,2,Y,Y',Z)$, with rational endpoints over
denominators dividing $10^4$ and $n$ decreasing from $36$ to $10$.  The
...
also checks that the first interval starts at $\frac{11}{10}$, that each
...
$[\frac{11}{10},\frac{139}{100}]$, and Proposition~\ref{prop:failinterval}
...
every root of $[\frac{11}{10},\frac{139}{100}]$, each time at the position
$2$ and a diagonal $n$ that moves with $r$.  The certificates were found by
...
$1.466$.  Whether every $1<r<2$ has a failure in some row, and whether
there is one for $1<r<\frac{11}{10}$, where the certificates would need
$n>36$, remain open.
...
Theorem~\ref{thm:failinterval} are in \texttt{tests/test\_rows.py}, with
the certificates of Theorem~\ref{thm:failinterval} in
\texttt{tests/second\_row\_cover.json}, of the repository
...
and~\ref{prop:thirdrowexact} and Theorem~\ref{thm:failinterval} are the
...
Proposition~\ref{prop:failinterval}, to
...
Theorem~\ref{thm:failinterval}, the comparison of
Proposition~\ref{prop:failinterval} for each listed tuple and the order of
their endpoints.  The tests
\texttt{test\_second\_row\_*} (including
\texttt{test\_second\_row\_hypothesis\_fails} and
\texttt{test\_second\_row\_fails\_on\_an\_interval}) and \texttt{test\_third\_row\_*} in
...
Theorem~\ref{thm:failinterval} are not part of any proof or test; they
