...
$r\in[\frac{21}{20},\frac{139}{100}]$, and in some row for every
$r\in[\frac{21}{20},\frac{19}{10}]$.
\end{abstract}
...
$r\in[\frac{21}{20},\frac{139}{100}]$ (Theorem~\ref{thm:failinterval}).
...
third row of a diagonal while holding in the second.  Above
$\frac{139}{100}$ the failures move to higher rows: the term
$1/\mu_r([2,k],L)$ of the exempted block $[2,k]$ is at least
$1/\nu_{k+1}(n-1)$, a prefix value of a longer prefix and a smaller degree,
with equality when its minimizer vanishes at $1$ (Lemma~\ref{lem:block}),
and finitely many
certificates show that the identity fails in some row $k\le384$ for
every $r\in[\frac{139}{100},\frac{19}{10}]$ (Theorem~\ref{thm:blockinterval}).
...
sets, shows that it can fail in the third row of a diagonal whose second
row satisfies it, and that it fails in some row at every root of an
interval reaching $\frac{19}{10}$.
...
every root of $[\frac{21}{20},\frac{139}{100}]$, each time at the position
...
So a failure at the block is a prefix value of a longer prefix and a
smaller degree exceeding every prefix value of the diagonal, as in
Proposition~\ref{prop:secondrowexact}(a) and~(d), and it occurs only in rows
$k<(\beta_r(n)+1)/r$, while Theorem~\ref{thm:longdiag} excludes rows
$k\ge\max\{K,\alpha(n-1)\}$.
...
Proposition~\ref{prop:rowvalue}(b) with $S=[2,k]$ gives the bound on
$V_r(n+k-1,k)$.
\end{proof}
...
Proposition~\ref{prop:blockinterval}: the sizes, the vertex test for $Z$ at
...
floating-point search, not part of any proof, finds block failures further
on: at $r=1.9$ for $n=14$ in the rows $210\le k\le254$ and for $n=17$ in
...
$r\to2$; at $r=2$ it holds in every row (Theorem~\ref{thm:allrowstwo}).
