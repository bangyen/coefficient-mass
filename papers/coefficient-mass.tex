...
sharp at $(2,3)$.  For monic multiples of $(x-2)^L$ the Jensen bound makes
the first rows exponentially large in $L$, and a separate certificate
argument gives $b_k\ge L-k+1$ for every row, which matters for the last
rows, where the Jensen bound says nothing.  The key tool compares the
...
when it is displaced, together with the largest node, the reciprocal of
the smallest root, strictly increases the tail, and iterating gives a product bound over
the initial run of consecutive zeros, with equality only when the zeros are
the first positive integers $1,2,\ldots$ with none skipped.  As a by-product, the same-sign hypothesis under which
Dubickas and Jankauskas evaluate the consecutive case holds at all distinct
nodes in $(0,1)$, making their reduced-height bound $H(P)\ge|P(1)|$ unconditional for
...
$\Lambda(F)\ge\sum_i\log r_i-\frac12\log(D+1)$ for monic $F$, at most of
order $L\log L$ at the first $L$ primes and vacuous once $\log D\gg L\log L$.
...
(Lemma~\ref{lem:slack}).  Its $\ell^1$ tail is bounded, uniformly over zero
...
holds unconditionally for distinct real roots greater than one.  What is new here is the tail estimate for
...
Theorem~\ref{thm:order}.  The zero bound is formalized for distinct zeros,
the sign changes at prescribed zeros being derived there from weak
alternation, and the worked examples are not formalized.
...
largest node $y_c=1/\rho_c$.  The hard step is the following bound: for the
...
(the remark after Proposition~\ref{prop:quadratic}).  The infimum of the mass over monic multiples is of order $L^2$
...
nothing about the last rows.  For monic multiples of $(x-2)^L$,
Theorem~\ref{thm:everyrow} at the end of this section proves every row
$b_k\ge L-k+1$.  This is weaker than \eqref{eq:jensen} for the first rows
...
The rows are not infima at every root set.  The companion
...
With $C,\ell,m,q,N$ as above, $H(C,q)\le\dfrac1{2(q+1)}$.
\end{lemma}
...
$H(C,q)\le H(C^\circ,q)$.  By the proof of Lemma~\ref{lem:holeint},
$H(C^\circ,q)=\int_0^1v^q\sum_{c\in C^\circ}|\pi_c|2^{-c}v^{2\ell+1-c}(1-v)^{c-1}\,dv$
...
With $C,\ell,m,q,N$ as above, if $\ell\ge1$ and $q\ge1$, then $T(C,q)\le\dfrac{\ell+1}{e\,\ell\,(q+1)}$.
\end{lemma}
...
\[
  T(C,q)\le(\ell+1)\binom m\ell\int_0^1t^\ell
...
\[
  (q+1)\,T(C,q)\le\frac{\ell+1}e\binom m\ell\int_0^\infty t^{\ell-1}(1+t)^{-m-1}\,dt
...
\[
  \Phi(Y)=H(C,q)+T(C,q)\le\frac1{q+1}\Bigl(\frac12+\frac{\ell+1}{e\,\ell}\Bigr)
...
individual rows.  For $\ell\in\{1,2\}$ the tail bound of Lemma~\ref{lem:holetail}
