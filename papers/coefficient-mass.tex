...
Jankauskas holds unconditionally for distinct real roots greater than one.
\end{abstract}
...
The standard lower bound for the coefficients of a multiple comes from the
Mahler measure.  A monic $F$ has $M(F)\ge\prod_ir_i$, and Landau's inequality
$M(F)\le\|F\|_2$ gives
$b_1(F)\ge\bigl((\prod_ir_i)^2-1\bigr)^{1/2}D^{-1/2}$.  This bound gains the
factor $\prod_i\frac{r_i}{r_i-1}$ over the row $k=1$ but loses $\sqrt D$, and it
controls a single coefficient.  For the mass it gives only
...
leading to them from the zero bound, that is, Sections~2 and~3 through
Corollary~\ref{cor:mass}, are formally verified in Lean~4 with Mathlib, in
the directory \texttt{CoefficientMass} of the repository
\href{https://github.com/bangyen/coefficient-mass}{bangyen/coefficient-mass}.
The formal proof of Corollary~\ref{cor:mass} depends only on the standard
axioms of Lean (propositional extensionality, choice and quotient
soundness), and the formal proofs follow the ones given here.  Two clauses are not
