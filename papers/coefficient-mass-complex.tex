...
(Lemma~\ref{lem:tropcount}), and the rows of Corollary~\ref{cor:annuli}
need a separation growing with the number of roots above: at the
separation $n_a$, $(x-q)(x+Kq)^K$ has $b_2$ below the row by a factor that
tends to $0$ (Proposition~\ref{prop:rowsneedn}), while for two annuli the
separation $n_2+1$ suffices, as a truncation of $\prod(1-x/\beta)^{-1}$ stays
away from $0$ below $T/n$ (Lemma~\ref{lem:trunc},
Theorem~\ref{thm:rowstwo}, Corollary~\ref{cor:rowstwo}).  For more annuli
the separation $n_a+9$ suffices (Theorem~\ref{thm:rowsk},
Corollary~\ref{cor:rowsall}), but the row $3$ needs both separations to grow
with the number of roots above (Proposition~\ref{prop:rowslower}).  The mass bound fails at
the separation $2$ (Proposition~\ref{prop:gaptwo}).
...
least fixed separation of the annuli for the mass is: it lies between $2$
and $81$, and whether the separation $9$ of Corollary~\ref{cor:annuli},
...
$n-(D-y)\le\lfloor0.13\,n\rfloor$ if $T\ge27r$.
\end{lemma}
...
For the last two claims let $0<c<1$, $M=\lfloor cn\rfloor+1>cn$ and
...
$T_a>(n_a+9)U_{a-1}$ (Corollary~\ref{cor:rowsall}).  Item~2 needs only the factor $9$, but past the first
...
The factor $81$ cannot be lowered to $2$ (Proposition~\ref{prop:gaptwo});
...
proved at the separation $9n_a$, and the factor $n_a$ cannot be dropped;
for two annuli $n_2+1$ suffices (Corollary~\ref{cor:rowstwo}), and in
general $n_a+9$ (Corollary~\ref{cor:rowsall}).
...
item~1, need grows linearly with $n_a$: it is at most $9n_a$, and not at
most $n_a$.

For two annuli this cancellation is the only obstruction, and the factor
$9n_2$ drops to $n_2+1$.  Dividing out the zeros above leaves a quotient
...
In the setting of Theorem~\ref{thm:annuli}, let $2\le k\le S$, $n=n_k$ and
$1\le j\le k$.  Let $r_1=1$, let $3U_{a-1}<r_a<T_a/3$ for $2\le a\le j$, and
suppose $3nr_j\le T_k$.  Then central indices $y_a$ of $F$ at $r_a$ satisfy
$y_1<\cdots<y_j$ and $D-y_j\ge n$, and
\[
  |f_{y_a}|\ >\ \frac{|f_D|}2\prod_{\beta\in A_{\ge k}}\frac{|\beta|}2\qquad(1\le a\le j).
\]
In particular, if $T_k>9n_kU_{k-1}$, then
$b_k(F)>\frac{|f_D|}2\prod_{\beta\in A_{\ge k}}\frac{|\beta|}2$.
\end{lemma}

\begin{proof}
The Order step of the proof of Theorem~\ref{thm:annuli} gives
$y_1<\cdots<y_j<D$.  Lemma~\ref{lem:tropcount} with $A=A_{\ge k}$, whose $n$
zeros have modulus at least $T_k\ge3nr_j$, gives $D-y_j\ge n$, so $D-y_a\ge
n$ for every $a\le j$.  As $r_a\ge1$, Lemma~\ref{lem:central} with
$A=A_{\ge k}$ gives
$|f_{y_a}|>\frac{|f_D|}2r_a^{D-y_a-n}\prod_{A_{\ge k}}\max(r_a,|\beta|/2)$,
which is at least the claim.  If $T_k>9n_kU_{k-1}$, take $j=k$ and
$3U_{k-1}<r_k<T_k/(3n)$; the $k$ positions $y_a$ bound $b_k(F)$.
\end{proof}

So the row $k$ of Corollary~\ref{cor:annuli}, item~1, needs the factor $9n_k$
only at the $k$-th separation.  Below it, a truncation at a zero of the
...
In the setting of Theorem~\ref{thm:annuli}, let $3\le k\le S$, $n=n_k$,
...
Let $r=xT_{k-1}$, so $3U_{k-2}<r<T_{k-1}/3$, $r\le xa$, and $3nr<na<T$.
...
\begin{corollary}[Rows at the separation $n_a+9$]\label{cor:rowsall}
In the setting of Theorem~\ref{thm:annuli}, let $3\le k\le S$.  The row
$b_k(F)>\frac{|f_D|}2\prod_{\beta\in A_{\ge k}}\frac{|\beta|}2$ holds if
$T_k>9n_kU_{k-1}$, or if $T_{k-1}>(n_{k-1}+9)U_{k-2}$ and
$T_k>(n_k+9)U_{k-1}$.  So if $T_a>(n_a+9)U_{a-1}$ for $2\le a\le S$, then
the rows and the mass bound of Corollary~\ref{cor:annuli}, item~1, hold.
\end{corollary}

\begin{proof}
The first case is Lemma~\ref{lem:rowsbelow}, which also covers the second
when $n_k=1$, as then $T_k>10U_{k-1}$.  Otherwise $n=n_k\ge2$; put $m=n+9$ and take $\alpha\in A_{k-1}$, so
...
$\gamma V>(\frac{11}{12})^3\frac{171}{520}>0.25>\frac{27}{208}$.  So the row
$k$ holds.  The rows $k=1,2$ hold by Theorem~\ref{thm:annuli} and
Corollary~\ref{cor:rowstwo}, and the rows give the mass as there.
\end{proof}
...
hold at $(n_a+9)U_{a-1}$; for any $c>3$ the same computation gives the row $k$
...
$T_1=U_1=q>3$ and $T_2=U_2=t>2U_1$, but
\[
...
amount, and its factor $81$ cannot be replaced by any $g\le2$.
\end{proposition}
...
Open: the least fixed separation for the mass, between $2$ and $81$, in
