...
$\prod_i(x-r_i)$, and hence a mass bound that charges the $i$-th root $i$ times.  We ask what
...
return, up to constants, when single moduli are separated by a factor $9$, by
an Archimedean Newton polygon argument; with several roots per annulus the
mass needs the separation $162$.  With $K_s$ roots of modulus in
...
that the cross terms are false as well, and that the sharp worst-case count charges a
...
$x^{2m}+x+a$ of Proposition~\ref{prop:noangle} do.  The angle mechanism of
...
split $m$ is not clean at $\alpha$, then $|F_{<m}(\alpha)|\ge\rho^m$, and if
