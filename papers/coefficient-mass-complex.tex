...
$c^2$: every $b_k$ and $\Lambda$ have the same infima over monic multiples.
...
it, and the real cross terms fail.  Real roots of both signs charge one root
...
degree.  In a sector of fixed half-width $\delta<\pi/2$, multiples of degree
$O(K)$ with $|f_D|\ge1$ at $K$ roots of modulus at least $R$ pay
$(\frac\pi\delta-o(1))K\log R$ as $K,R\to\infty$, sharply.
\end{abstract}
...
evaluation.  The file \texttt{tests/test\_complex.py} in the
repository of~\cite{coefficient-mass} only checks the statements, on seeded
random integer multiples and on the extremal examples, each group with a
control showing that a false variant is caught.  Polynomial products,
divisibility, coefficient magnitudes, central indices, Rouch\'e comparisons
and Sturm counts of real zeros are computed in exact integer and rational
arithmetic; floating point enters only in evaluating logarithms, in the
right-hand sides of mass bounds, the constant $E$ and the tropical roots.
