...
(Lemma~\ref{lem:twozeros}, Corollary~\ref{cor:twonear}).  The mass bound fails with
...
and~\ref{cor:rowsall}).  Item~2 needs only the factor $9$, but past the first
...
Corollary~\ref{cor:fewabove}, and at $n_a+9$ by Corollary~\ref{cor:rowsall},
and the factor $n_a$ cannot be dropped; for two annuli $n_2+1$ suffices
...
(Corollaries~\ref{cor:fewabove} and~\ref{cor:rowsall}), and not at most
$n_a$.
...
large, and at $\lambda=0$ this is Proposition~\ref{prop:rowsneedn}.  The
best constant $c$ in $b_2(F)\ge c\lambda(1-\frac1{|\alpha|})|f_D|\prod_{\beta\in A}|\beta|$
lies in $[\frac18,1]$.  For $n=1$ the truncations alone give only
$\lambda^2$, as $|S_m(\alpha)|$ can be close to $1/\lambda$; the proof uses
both zeros instead.
...
$\varepsilon>0$.  With $S=2$ and $n_2\le8$ this also gives the mass bound of
...
Let $h\ge1$, $K\ge18h$ and $p\ge2^K$ be integers, $q_1=2h(K+h)p$,
$q_2=K(K+1+2h)p$, $t=(K+h)q_2$, and $F=(x-q_1)(x-q_2)(x+t)^K$.  Then
Theorem~\ref{thm:annuli} applies to the monic $F\in\mathbb Z[x]$ with $S=3$,
$A_1=\{q_1\}$, $A_2=\{q_2\}$ and $A_3$ the $K$-fold zero $-t$, so that
$T_3=(n_3+h)U_2$ and $T_2<(\frac{n_2}{2h}+1)U_1$, and
\[
  f_2=0,\qquad b_3(F)\ \le\ \frac{4t^K}{q_2}\ <\ \frac12\prod_{\beta\in A_3}\frac{|\beta|}2 .
\]
So the row $k=3$ of Corollary~\ref{cor:annuli}, item~1, fails; and as
...
$b_3(F)\ge\varepsilon|f_D|\prod_{\beta\in A_3}|\beta|$.
\end{proposition}

\begin{proof}
Here $n_3=K$ and $n_2=K+1$, $T_3=U_3=t=(K+h)q_2$, and
$T_2/U_1=q_2/q_1=K(K+1+2h)/(2h(K+h))$, which is below
$(K+1+2h)/(2h)=\frac{n_2}{2h}+1$ and above $9$, as $K\ge18h$ gives $K^2\ge18hK$
and $K+2hK>18h^2$; and $T_1=q_1>3$.  With $(x+t)^K=\sum_i\binom Kit^{K-i}x^i$,
for $2\le i\le K+1$,
\[
...
bounds $b_3(F)$, and $4/q_2<2^{-K-1}$ as $q_2\ge18\cdot21\cdot2^K$.
\end{proof}

In this family $\lambda=1-n_3U_2/T_3=h/(K+h)$ and $\xi=3U_1/T_2$ is about
$6h/K$, while Theorem~\ref{thm:rowsk} needs about $\lambda>\xi$; so its
condition is sharp up to a constant factor.  Whether the separation
$T_a>(n_a+1)U_{a-1}$ for every $a$ gives all the rows is open.
