...
multiplier and explicit certificates (Theorem~\ref{thm:familylow}).  These
...
(Theorem~\ref{thm:familylow}); at $(r,5,7)$ it fails for every $1<r<2$
...
proof.  A floating-point search over zero sets in $\{1,\ldots,40\}$, at the
...
($N=15$ at $r=1.06$), which would make infinitely many pieces on
$(1,\rho_1)$.
...
\item The infimum along $(r,3,5,7)$ for $1<r<\rho_1=1.0745\ldots$, where
Theorem~\ref{thm:family} gives only the upper bound $\tfrac{209}4$.
Theorem~\ref{thm:familylow} gives it on $[\rho_1,\tfrac{13}{10}]$ in seven
pieces; below $\rho_1$ the pieces seem to accumulate at $r=1$ (the remark
after that theorem).  Precisely: is the number of pieces on $(1,\rho_1)$
infinite, does the worst placement tend to $\infty$ as $r\to1^+$, and what
is $\lim_{r\to1^+}\inf_Qb_2(QP_r)$?
\end{itemize}
