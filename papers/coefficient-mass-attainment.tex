...
$1/\tau^*$.  The infimum is never attained.
\end{abstract}
...
\emph{The value.}  Record the positions of the exempt coefficients by their
distances below the leading coefficient, a \emph{placement}
$X\subset\N_{>0}$ with $|X|=u$.  A \emph{certificate} for $X$ is an
exponential sum $v_d=\sum_i\lambda_ir_i^{-d}$ with $v_0=1$ and $v|_X=0$
(for repeated roots, an element of the Hermite space of the roots), and
$\tau(X)$ is the least tail $\sum_{d\ge1}|v_d|$ of a certificate for $X$.
For $1<r_1\le\cdots\le r_L$ the infimum is $1/T$ with
$T=\sup_{|X|=u}\tau(X)$ (Theorem~\ref{thm:valuerep}).  As the exempt
...
For distinct roots above $1$ the supremum $T$ is attained, by a placement or
at a level where some exempt coefficients have escaped
...
$\tfrac{29}{20}\le r\le r_c$ the infimum is $48(r-1)(15r+71)/(49r-34)$
(Theorem~\ref{thm:family}).  Both proofs end in a finite exact computation,
over $\mathbb Q$ and over $\mathbb Q(r)$ respectively, described in the
statement on computer assistance before the appendix.  Along $(r,5,7)$ the
...
Three arguments end in exact computations that are steps of the proof.  In
...
$x=2,4,6,\ldots,10$, with their comparisons with $\theta$, are computed over
$\mathbb Q$ in \texttt{test\_value\_eleven\_tenths}.  In
Theorem~\ref{thm:family}, the tails of the full certificates with zero sets
$\{1,3,4\}$ and $\{1,2,5\}$ and the closed form of
$2\sum_{d\le5}|H_d|-\Tail(H)$ are identities in $\mathbb Q(r)$, verified with
$r$ an indeterminate in \texttt{test\_family\_threshold};
Appendix~\ref{sec:familydata} lists the solutions of the linear systems, from
which they can also be checked by hand.  The statement after
...
truncation of an exact rational.  The other computations only check steps
...
(\texttt{test\_family\_threshold}); and
Proposition~\ref{prop:cutoffsharp} (\texttt{test\_cutoff\_sharp}).  The file
also holds tests used in no proof: seeded random exact checks of
...
and revising the text, the proofs, the verification code and the Lean
formalization.  The author has checked the mathematics and is responsible
for the content.
