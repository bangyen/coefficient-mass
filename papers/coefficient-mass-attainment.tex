...
$(r-1)^{\log(5/3)/\log3}$.  The infimum is never attained.
\end{abstract}
...
transfers them to $r$ near $1$.  These
...
(Theorem~\ref{thm:familyone}); at $(r,5,7)$ it fails for every $1<r<2$
...
(Theorem~\ref{thm:familyone} and Corollary~\ref{cor:familyone}).
...
seven pieces, and Theorem~\ref{thm:familyone} with
Corollary~\ref{cor:familyone} shows that infinitely many pieces accumulate
at $r=1$, that the worst placement is $\log_3\frac1{r-1}+O(1)$, and that
$\inf_Qb_2(QP_r)-24$ is of exact order $(r-1)^\alpha$,
$\alpha=\log(5/3)/\log3$.  Open: the pieces themselves on $(1,\rho_1)$
(numerically the zero set minimising $\tau(\{x\})$ is $\{1,c,x\}$ with
$c\in\{3,4\}$ or $\{1,x,z\}$ with $z$ growing as $r$ decreases, and by
Theorem~\ref{thm:familyone}(b) its cost tends to that of $\{1,x\}$ at
$(3,5,7)$), and whether $(\inf_Qb_2(QP_r)-24)(r-1)^{-\alpha}$ has a limit
as $r\to1^+$, or oscillates as the integer worst placement moves.
\end{itemize}
...
Four computations are computer-assisted: steps of the proofs of
...
lists the tails, the breakpoint polynomials and one witness.  The statement after
...
truncation of an exact rational or of a zero of an explicit polynomial.  The
floating-point search reported after Theorem~\ref{thm:familylow} is
exploration, used in no proof and not part of the tests.  The other computations only check steps
...
Theorem~\ref{thm:familyone} are by hand.  The file
