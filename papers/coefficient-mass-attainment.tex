...
and $\tfrac{400}3\cdot18^{-\alpha}$.  The infimum is never attained.
\end{abstract}
...
$\tfrac{400}3\cdot18^{-\alpha}$ (Theorem~\ref{thm:familyosc}).  These
...
(Theorem~\ref{thm:familyosc}); at $(r,5,7)$ it fails for every $1<r<2$
...
upper end of its piece to $1$ at the lower end, where $\{1,4,z+1\}$ takes
over with the same tail; at $\rho_7$ the entries $s_3$ of $\{1,3,4\}$ and
...
closed form.  A floating-point search over zero sets in $\{1,\ldots,40\}$, at the
...
$\log r\le\varepsilon$ and $\varepsilon\log\tfrac{945}{2\varepsilon}\le\tfrac1{100}\log47250<\tfrac{27}{250}$,
$r^X<e^{1/10}<\tfrac{111}{100}$, so $3^X<\tfrac{111}{100}\cdot\tfrac{945}{2\varepsilon}<\tfrac{525}\varepsilon$
...
it, and there the transition is slow: at $r=\tfrac{101}{100}$, where
$3^7\varepsilon=21.87$, a floating-point search finds the worst placement
$\{7\}$, with zero set $\{1,4,7\}$, rather than $x^*=6$.
...
\item The infimum along $(r,3,5,7)$ for $1<r<\rho_1=1.0745\ldots$ in closed
form.  Theorem~\ref{thm:familylow} gives it on $[\rho_1,\tfrac{13}{10}]$ in
seven pieces, Theorem~\ref{thm:familyone} with
Corollary~\ref{cor:familyone} shows that infinitely many pieces accumulate
at $r=1$ and that $\inf_Qb_2(QP_r)-24$ is of exact order $(r-1)^\alpha$,
$\alpha=\log(5/3)/\log3$, and Theorem~\ref{thm:familyosc} shows that
$(\inf_Qb_2(QP_r)-24)(r-1)^{-\alpha}$ has no limit, its limit points filling
$[80\cdot18^{-\alpha},\tfrac{400}3\cdot18^{-\alpha}]$; in its notation, for
$r$ near $1$ with $\delta\le\omega\le1-\delta$ the worst placement is $\{x^*\}$, with
minimising zero set $\{1,x^*,z\}$ for some $z\ge x^*+3$.  Open: the pieces
themselves, that is, which $z$ (the $z$-th piece ends where the witness
entry $s_z$ of Lemma~\ref{lem:eighteen}(a) reaches $\pm1$), an explicit
$r_\delta$ in Theorem~\ref{thm:familyosc}(a), and the transition near the
jumps, where the placements $x^*$ and $x^*+1$ compete (numerically the zero
set minimising $\tau(\{x^*+1\})$ there is $\{1,c,x^*+1\}$ with
$c\in\{3,4\}$; at $r=\tfrac{101}{100}$ the worst placement is $\{7\}$, not
$x^*=6$).  In particular: does $\Delta(r)-\Phi(\omega(r))\to0$ uniformly on
$0<\omega\le1-\delta$; and when $3^x(r-1)\to\kappa>18$ with $x\to\infty$,
does $\tau(\{x\})$ tend to $\min_{c\ge2}(t(c)+\ell(c)/\kappa)$, the limit of
the bounds of Lemma~\ref{lem:eighteen}(b)?
\end{itemize}
...
Five computations are computer-assisted: steps of the proofs of
...
$11\le x\le20$.  The statement after
...
(\texttt{test\_family\_threshold} and \texttt{test\_family\_window}); and
...
Theorem~\ref{thm:familyone} are by hand.  In
...
in the proof of Theorem~\ref{thm:familyosc} are by hand.  The file
