...
\begin{enumerate}
\item If $F=A(x^N)$ with $N\ge1$ and $A\in\mathbb Z[y]$ is nonzero and
...
item~2 says: the middle term of the display charges $\log n_j$ once for every
root of $F/x^e$ that is at least as divisible by $p$ as $\alpha_j$, other
than $\alpha_j$ itself, and roots of $F/P$ need not be; at $F=P$ it is
...
equal to a constant $-N$ if and only if $N\lambda_j=\Lambda Q'(w_j)$ for
every $j$.
...
if $h_1\ne0$ the $\alpha_j$ have distinct norms.  Whether the number of
Gaussian points above the axis in one level set of a polynomial with small
nonleading coefficients is bounded independently of $\rho_{\min}$, whether
one norm carries at most two of them, and whether $\Lambda(F)\ge cK^2\log R$
holds for all integer multiples at Gaussian roots in a wide sector, remain
open.
