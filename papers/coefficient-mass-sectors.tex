...
level set of a polynomial with small nonleading coefficients.
\end{abstract}
...
Whether such a level set holds boundedly many Gaussian points above the
axis is open.
...
sets (Proposition~\ref{prop:gaussheavy}).
...
above the axis can share such a level set: $(y^3-y)^2$, with nonleading
coefficients $-2$ and $1$, takes the value $-270400$ at $\pm7+4i$ and $8i$,
since $y^3-y$ is $\pm520i$ there, and these points have
$\rho_{\min}=8>4$.  Whether the number of Gaussian points above the
axis in one level set
of a polynomial with small nonleading coefficients is bounded, and whether
$\Lambda(F)\ge cK^2\log R$ holds for all integer multiples at Gaussian roots in
a wide sector, remain open.
