...
rows.  The second row is a minimum over finitely many exempted positions,
each term attained by a polynomial with integer zeros whose optimality, for
...
It is not attained in general.  The second row $k=2$ is a minimum over
finitely many exempted positions (Theorem~\ref{thm:secondrow}), and exact
...
the limit $\nu_1(n)$, so the second row is a minimum over finitely many
positions (Theorem~\ref{thm:secondrow}).  Each term is $\Phi_r(Z)$ for a set
