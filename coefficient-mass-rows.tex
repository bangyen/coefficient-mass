...
also prove it whenever $k\ge\max(K,\alpha(n-1))$, where $\alpha\ge1$ is any
constant with $\rho(\alpha)<1$ for an explicit function $\rho$ depending on
$r$, and $K$ depends on $r$ and the chosen $\alpha$
...
The lower bounds come from certificates.  We first extend the
displaced-zero tail bound of \cite{coefficient-mass} to nodes without the
cutoff $y_i\le\frac12$ (that is, $\rho_i=1/y_i\ge2$ there).  At the root $r$
it bounds the cost of deleting the largest zero (Lemma~\ref{lem:confdelr}),
which is used in the remark after Theorem~\ref{thm:crossing}; the last row
...
$X\in\{V,G\}$ and $\Sigma_G=\sum_{1\le d<z}|G_d|$, the notation of the proof of
\cite[Theorem~2.2]{coefficient-mass}.  That proof derives the identity
\cite[eq.~(2)]{coefficient-mass},
$\Tail(V)-\Tail(w)=\xi(2\Gamma_V(z)-\Gamma_G(z)+\Sigma_G)$, and the first
inequality of \cite[eq.~(3)]{coefficient-mass},
$\Gamma_G(z)\le\Gamma_V(z)/(1-y_c)$, from the zero bound of
\cite[Section~2]{coefficient-mass}, \cite[Lemma~2.3]{coefficient-mass}
(which needs no upper bound on the nodes) and $y_c<1$ only; the cutoff
$y_c\le\frac12$ enters there only through the second inequality of
\cite[eq.~(3)]{coefficient-mass}.  So both hold here.  If $y_c\le\frac12$,
that second inequality holds as well, and the proof of
\cite[Theorem~2.2]{coefficient-mass} gives
...
$\rho_k=\theta\,(1+(n-1)/(M_k+1))^d\sqrt{1+1/k}$.  Then $\Xi(k+1)\le\rho_k\Xi(k)$,
and $\rho_k$ does not increase with $k$.  So if $\Xi(k_1)\le3(2-r)^2/r^2$ and
...
$\Xi(k+1)\le\rho_k\Xi(k)$.  Since $M_k$ does not decrease, neither does
...
In Propositions~\ref{prop:secondrowexact} and~\ref{prop:thirdrowexact} each
part fixes a root $r$ and a top-row degree $n$, and a named set of integers
...
satisfies the bound.  In each part $p=r_{Y_n}$, and $\sigma_0$ is the least
...
$\eta_2(15)=\Phi_r(Z_2(15))$, and with $S=\varnothing$ that
...
$Z_\varnothing=Y$.  For a child $N\cup\{\sigma\}$ of a node $N$ with kept
set $Z_N$ it forms these candidates: $Z_N$ itself if $\sigma\in Z_N$;
otherwise $Z_N$ with one of the three elements of $Z_N\setminus N$ nearest
to $\sigma$ replaced by $\sigma$, and $Z_N\cup\{\sigma\}$; and in either
...
$\mu_r(\{2,3\},16)=\Phi_r(Z_{2,3}(14))$ and $\nu_1(14)=\Phi_r(Y_{14})$, which is
...
second.  In (b) the certified optimum $Z_{2,3}(14)$ contains $[1,3]$, as the
certified optima in Proposition~\ref{prop:secondrowexact}(a) and~(d) contain
$[1,\sigma]$; in (a) $Z_{2,3}(12)$ omits $1$.  The identities at
