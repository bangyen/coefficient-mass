...
\[
  \Tail(w)\le\Tail(V)+\mu\Gamma_V(z)\Bigl(\frac1{1-y_c}-2\Bigr),
\]
where $\mu=|V_z|$ and $z=\max Z$ as there, the bracket is positive, and
$\mu\Gamma_V(z)=\sum_{d\ge z}|V_d|\le\Tail(V)$ (if $V_z=0$ then $w=V$), so
...
every $y\in Z\setminus S$.  Conversely, for every finite
...
For the converse take integers $D_j\to\infty$ with
...
$\sum_{s\le M}|r_Z(s)|r^{-s}=\lim_j\sum_{s\le M}|r_{W_j}(s)|r^{-s}\le
\mu_r(S,L)$, since truncated sums are at most full ones.  So
...
lemma is finite.  By the converse, every $\mu_r(S,L)$, in particular every
