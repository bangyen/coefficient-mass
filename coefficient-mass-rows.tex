...
$\nu_1(n)$, $\nu_2(n)$ and $\eta_\sigma(n)$ by exact line search over integer
zero sets, one zero at a time (in exact arithmetic a set admitting no
...
identity held at $(r,n+1,2)$, with the worst position $\sigma=1$, although
$\Lambda_2(n)>\nu_2(n)$ at several of them (for instance $r=\frac54$,
$7\le n\le10$), so Theorem~\ref{thm:onepoly} does not explain it.
...
with $\beta_r(n)>5$, by at most $19\%$, and with worst position $\sigma=2$
