...
this sharpens the bound $b_k\ge L-k+1$, strictly for some rows.  The proof reduces each row to an
$\ell^1$ problem for polynomials with prescribed integer zeros, handles the last
exempted position, wherever it lies, by one crossing step at the first gap of
a minimizer, inducting on the number of exempted positions,
and bounds the cost of that step by first-order optimality.  For $1<r<2$ the
rows are not all top rows.  We show that they are given by
prefix-constrained problems whenever the top row $\beta_r(L-k+1)$ of the diagonal is at most one, and on each diagonal $L-k=\mathrm{const}$ for all but finitely many
rows.  The second row $V_r(L,2)$ is the minimum of the top row $\beta_r(L-1)$ of its diagonal and finitely many terms, one
for each nearby exempted position.  More generally, row $k\ge2$ is the minimum
of row $k-1$ on the same diagonal and finitely many terms, one for each
exempted set in a finite tree cut off by thresholds for several far zeros.
Each term is attained by a polynomial with integer zeros whose optimality, for
rational $r$, is a finite exact check.  At $r=\frac54$ and $r=\frac43$ this shows that the
prefix description fails in general, and at $r=\frac43$ that it can fail in
the third row while holding in the second row of the same diagonal.
\end{abstract}
...
$\beta_2(3)\ge\frac{40}{11}>3$.  Earlier
...
For $1<r<2$ the rows are not all top rows.  The last row is
$V_r(L,L)=(r-1)^L<\beta_r(1)$ for $L\ge2$ (Corollary~\ref{cor:lastrowr}).
The natural comparison values are the prefix values $\nu_i(n)$: the least
weighted $\ell^1$ norm $\sum_{s\ge1}|q(s)|r^{-s}$ of a polynomial $q$ with
$q(0)=1$ and $\deg q\le n+i-2$ that vanishes on $[1,i-1]$.  They satisfy
\[
...
It is not attained in general.  The second row $V_r(n+1,2)$ is the minimum of the top
row $\beta_r(n)$ and finitely many exempted-position terms (Theorem~\ref{thm:secondrow}), and exact
certificates give $V_{5/4}(16,2)=1/\nu_3(14)<\min_{i\le2}1/\nu_i(15)$ and
$V_{4/3}(19,2)<\min_{i\le2}1/\nu_i(18)$
(Proposition~\ref{prop:secondrowexact}).  Every row $k\ge2$ is likewise
the minimum of $V_r(L-1,k-1)$ and finitely many exempted-set terms
(Theorem~\ref{thm:finiterows}), and exact certificates give
$V_{4/3}(16,3)<V_{4/3}(15,2)=\min_{i\le3}1/\nu_i(14)$
(Proposition~\ref{prop:thirdrowexact}).
...
the integers in that range.  For a finite set
...
displaced-zero tail bound without the cutoff $y_i\le\frac12$ (that is, $\rho_i=1/y_i\ge2$ in \cite{coefficient-mass}).
...
sequence $w_d=\sum_ia_iy_i^d$ with these values, $\Tail(w)=\sum_{d\ge1}|w_d|$,
...
\]
Moreover, if $Z\ne[1,c-1]$ and $V$ is the sum on $y_1,\ldots,y_{c-1}$ with
$V_0=1$ and $V|_{Z\setminus\{\max Z\}}=0$, then
$\Tail(w)\le\max(1,\varphi_c)\Tail(V)$.
\end{theorem}
...
no cutoff.  In the displaced step, with $z=\max Z$ and $V$ as in the
statement, let $G$ be the sum on all $c$ nodes with
$G|_{\{0\}\cup(Z\setminus\{z\})}=0$ and $G_z=1$, put $\xi=|V_z|$, so that
$w=V-V_zG$, and put $\Gamma_X(z)=\sum_{d\ge z}|X_d|/|X_z|$ for
$X\in\{V,G\}$ and $\Sigma_G=\sum_{1\le d<z}|G_d|$, all as in the proof of
\cite[Theorem~2.2]{coefficient-mass}.  The sign comparison, the splitting
identity and the first correction-ratio inequality in the proof of
\cite[Theorem~2.2]{coefficient-mass},
$\Gamma_G(z)\le\Gamma_V(z)/(1-y_c)$, use only the zero bound,
...
$\Tail(w)<\Tail(V)$.  If $y_c>\frac12$, then $\Sigma_G\ge0$ and
the splitting identity give
\[
  \Tail(w)\le\Tail(V)+\xi\Gamma_V(z)\Bigl(\frac1{1-y_c}-2\Bigr),
\]
where the bracket is positive, and
$\xi\Gamma_V(z)=\sum_{d\ge z}|V_d|\le\Tail(V)$ (if $V_z=0$ then $w=V$), so
$\Tail(w)\le(\frac1{1-y_c}-1)\Tail(V)=\varphi_c\Tail(V)$.  In both cases
$\Tail(w)\le\max(1,\varphi_c)\Tail(V)$.  The set $Z\setminus\{\max Z\}$ has
the same leading run $\ell$ and $\ell+1\le c-1$, so the induction hypothesis
for $V$ on $y_1,\ldots,y_{c-1}$ gives the bound.
\end{proof}
...
$Z\ne[1,|Z|]$ put $c=|Z|+1$ and run the limit in the proof of
\cite[Lemma~4.6]{coefficient-mass} with
the nodes $y_i=\frac1r-(c-i)\varepsilon$ and the comparison
$\Tail(w^\varepsilon)\le\max(1,\varphi_c)\Tail(V^\varepsilon)$ of
Theorem~\ref{thm:tailgen} for its sums $w$, $V$ on these nodes; the
domination of the divided differences, with $h_j$ the complete homogeneous
...
\end{equation}
It is false for some $1<r<2$ (Proposition~\ref{prop:secondrowexact}).
...
$\sum_ss|r_W(s)|r^{-s}$, the shift costs at most the factor $1/r$.  The
...
second row for every $n$ and shows that the identity can fail, and
...
This subsection treats $k=2$ with no condition on $\beta_r(n)$.  The
exempted set is one position $\sigma$, and a polynomial vanishing there is
$(1-s/\sigma)p$ with $p\in\mathcal Q_n$, so we write
\[
...
finitely many position terms (Theorem~\ref{thm:secondrow}).  Each term is $\Phi_r(Z)$ for a set
$Z\ni\sigma$ of $n$ integers, and such a $Z$ can be certified exactly
(Lemma~\ref{lem:vertexopt}).  At $r=\frac54$ and $r=\frac43$
this determines rows on both sides of the prefix identity
(Proposition~\ref{prop:secondrowexact}).
...
every $y\in Z\setminus S$.  Moreover, for every finite
...
polynomial $P$.  So for rational $r$ both $\Phi_r(Z)$ and the $G_y$ are
...
Let $r=\frac54$ in (a)--(c).  In the named sets below $Y_n$ has $n-1$
elements, $Y'_n$ has $n$, and $Z_\sigma$ has $n$ and contains $\sigma$,
where $n$ is the argument of $\nu_1(n)$ in the part concerned.  Let
\begin{align*}
  Z_2&=\{1,2,5,8,12,17,22,28,35,44,53,65,78,94,115\},\\
  Y_{15}&=\{1,3,6,9,13,18,24,30,38,47,58,71,87,107\},\\
  Y_{13}&=\{1,3,6,10,15,21,27,36,45,57,72,91\}.
\end{align*}
\begin{enumerate}
\item[(a)] $V_r(16,2)=1/\Phi_r(Z_2)=1/\nu_3(14)=6.5694\ldots$, while
$\min_{i\le2}1/\nu_i(15)=\beta_r(15)=1/\Phi_r(Y_{15})=7.6492\ldots$.
\item[(b)] $V_r(17,3)\le1/\Phi_r(Z_2)<\beta_r(15)=\min_{i\le3}1/\nu_i(15)$.
...
$(x-\frac54)^{13}$.
\item[(d)] Let $r=\frac43$,
$Z_3=\{1,2,3,6,8,11,15,18,23,28,34,41,48,57,66,78,91,109\}$ and
$Y_{18}=\{1,2,4,6,9,12,16,20,25,30,36,44,52,61,72,85,103\}$.  Then
$V_r(19,2)=1/\eta_3(18)=1/\Phi_r(Z_3)=1/\nu_4(16)=41.38\ldots$, while
$\eta_2(18)<\nu_1(18)$ and
...
\begin{proof}
All numbers below are exact rationals, computed as described after
Lemma~\ref{lem:vertexopt}; the computation is in
\texttt{tests/test\_rows.py}.

(a) Lemma~\ref{lem:vertexopt} with $S=\{2\}$ shows
$\eta_2(15)=\Phi_r(Z_2)$, and with $S=\varnothing$ that
$\nu_1(15)=\Phi_r(Y_{15})$, which is less than $\Phi_r(Z_2)$.  The tail
condition $2R_p(\sigma_0)\le\Psi_r(p)$ holds for $p=r_{Y_{15}}$ at
$\sigma_0=107$ (an admissible $\sigma_0\ge\sigma_*$; the least is $\sigma_*=25$, Section~\ref{sec:finiterows}), so $\eta_\sigma(15)\le\Phi_r(Y_{15})$ for $\sigma\ge107$ by
the remark after Theorem~\ref{thm:secondrow}.  For every
$\sigma<107$ other than $2$, one of the following sets $Z\ni\sigma$ of $15$ integers has
$\Phi_r(Z)\le\Phi_r(Z_2)$:
...
$\sup_\sigma\eta_\sigma(15)=\Phi_r(Z_2)$, and
Proposition~\ref{prop:rowvalue}(b) gives $V_r(16,2)=1/\Phi_r(Z_2)$.  Since
$[1,2]\subseteq Z_2$,
$\eta_2(15)\le\nu_3(14)=\mu_r([1,2],16)\le\Phi_r(Z_2)=\eta_2(15)$.  Finally
...
tail condition holds for $p=r_{Y_{13}}$ at $\sigma_0=91$, and every
...
(d) As in (a): Lemma~\ref{lem:vertexopt} gives $\eta_3(18)=\Phi_r(Z_3)$
and $\nu_1(18)=\Phi_r(Y_{18})<\Phi_r(Z_3)$; the tail condition holds for
$p=r_{Y_{18}}$ at $\sigma_0=103$; and every $\sigma<103$ other than $3$
lies in a set of $18$ integers with $\Phi_r\le\Phi_r(Z_3)$, found among
...
$\Phi_r(Y'_{18})<\nu_1(18)$, and $[1,3]\subseteq Z_3$ gives
$\eta_3(18)=\nu_4(16)=\mu_r([1,3],19)$ as in (a).
\end{proof}

Neither Theorem~\ref{thm:onepolyrows} nor Theorem~\ref{thm:longdiag}
reaches (c), since both place the minimum at $i=k$.  In (a) and (d) the
row is attained at a position $\sigma$ with an optimum containing all of
$[1,\sigma]$.  So there the second row equals the prefix value
$1/\nu_{\sigma+1}(n+1-\sigma)$, of a longer prefix and a smaller degree than
the prefix identity allows, although only one position is exempted.

\paragraph{Numerical findings.}
The following was found in floating point and is not proved.  We computed
$\nu_1(n)$, $\nu_2(n)$ and $\eta_\sigma(n)$ in floating point by line
search over integer zero sets, one zero at a time (in exact arithmetic a set admitting no
improving move is optimal, by the proof of Lemma~\ref{lem:vertexopt}), for
$r\in\{\frac65,\frac54,\frac{13}{10},\frac43,\frac75,\frac32,\frac85,\frac53,
\frac74,\frac95,\frac{19}{10}\}$, $2\le n\le20$ and $2\le\sigma\le8$, and for
$\sigma\le30$ at $r\in\{\frac{11}{10},\frac54,\frac43,\frac32,\frac74,
\frac{19}{10}\}$, $n\le10$, and $\sigma\le20$ at
$r\in\{\frac54,\frac43,\frac32\}$, $11\le n\le16$.  On the grid, ``the
identity held'' means that no computed $\eta_\sigma(n)$ exceeded
$\max\{\nu_1(n),\nu_2(n)\}$; the positions between $8$ and $\sigma_*$ were
not all computed there.
\begin{itemize}
\item Whenever $\nu_2(n)\ge\nu_1(n)$ (45 of the 209 grid points), the prefix
identity held at $(r,n+1,2)$, with the worst position $\sigma=1$.
\item The identity failed only for $r\le\frac43$: at $r=\frac65$ for
$17\le n\le20$, at $r=\frac54$ for $14\le n\le18$, at $r=\frac{13}{10}$ for
$n\in\{12,13,14,20\}$ and at $r=\frac43$ for $n\in\{11,12,13,18\}$, always
with $\beta_r(n)>5$, with the ratio of $\min_{i\le2}1/\nu_i(n)$ to
$V_r(n+1,2)$ at most $1.19$ among the computed positions (uncomputed
positions could only raise it), and with worst position $\sigma=2$
except at $(\frac{13}{10},20)$ and $(\frac43,18)$, where it is $\sigma=3$.
\item At the other $147$ grid points, including all with $r\ge\frac75$,
the identity held with $\nu_2(n)<\nu_1(n)$, so the second row was the top
row $\beta_r(n)$, as in Proposition~\ref{prop:secondrowexact}(c).
\end{itemize}
So at a fixed root the failures at $\sigma=2$ come in runs of consecutive
$n$, and those at $\sigma=3$ appear later and in isolation.  We
conjecture that the prefix identity holds at $(r,n+1,2)$ whenever
$\nu_2(n)\ge\nu_1(n)$.  Whether a worst finite position is always small,
and whether failures occur for every $1<r<2$ or only near $1$, we do not
know.
...
\item[(b)] If $t>0$, $\Delta_{q,m}(t)\le0$, and $F\subset\N_{>0}$ is finite
with $|F|\le m$ and all elements at least $t$, then
$\Phi_r(q\,r_F)\le\Phi_r(q)$.
\end{enumerate}
...
(b) Let $F\ne\varnothing$ and $u=\min F\ge t$, so $\Delta_{q,m}(u)\le0$ by
(a).  Every $f\in F$ has $|1-s/f|\le\max\{1,s/f\}\le\max\{1,s/u\}$, so
$|r_F(s)|\le|1-s/u|\max\{1,s/u\}^{m-1}$ for $s\ge1$.  Hence
$u(|r_F(s)|-1)\le\phi_m(s,u)$: for $s\le u$ the left side is at most
$(u-s)-u$, and for $s>u$ at most $(s-u)(s/u)^{m-1}-u$.  Multiplying by
$|q(s)|r^{-s}$ and summing gives
$u\bigl(\Phi_r(q\,r_F)-\Phi_r(q)\bigr)\le\Delta_{q,m}(u)\le0$.  For $m=1$,
...
$F=S\setminus N_j$ has $m-j$ elements, all at least $\sigma_{j+1}$, so
Lemma~\ref{lem:farzeros}(b) gives $\Phi_r(q\,r_F)\le\Phi_r(q)$.  The
polynomial $q\,r_F$ has degree at most $n+m-1$, takes the value $1$ at $0$
...
$r$ and $q=r_Z$, $\Delta_{q,m}(t)$ is a rational number computable exactly
...
\begin{align*}
  ...
  Z_2&=\{1,2,5,8,12,16,21,28,35,44,56,71\},\\
  Z_{2,3}&=\{2,3,4,7,10,15,19,25,32,40,49,61,77\}.
\end{align*}
Then $V_r(14,3)=1/\Phi_r(Z_{2,3})=7.7168\ldots$,
$V_r(13,2)=1/\Phi_r(Z_2)=1/\nu_3(11)=8.5482\ldots$ and
$\min_{i\le4}1/\nu_i(12)=\beta_r(12)=1/\Phi_r(Y_{12})=8.8501\ldots$.  So
$V_r(14,3)<V_r(13,2)<\min_{i\le3}1/\nu_i(12)$: the prefix identity fails at
...
\begin{proof}
All numbers below are exact rationals, and the computation is in
\texttt{tests/test\_rows.py}.  Each lower bound for a row comes from
Theorem~\ref{thm:finiterows}(b) with $q_\varnothing=r_Y$ for the set $Y$
named in that part, $q_N=r_{Z_N}$ for sets $Z_N\supseteq N$ of at most
...
$\Phi_r(Z)\le B$.  The test builds the tree.  For each child
$N\cup\{\sigma\}$ it tries $Z_N$ with one of the three elements outside $N$
nearest to $\sigma$ replaced by $\sigma$, or with $\sigma$ added, and the
fixed set of $n+|N|$ integers (\texttt{\_FULL12}, \texttt{\_FULL14} and
\texttt{\_FULL8} in the test, for (a)--(c)) with each position of
$N\cup\{\sigma\}$ that it misses put in place of one of the two elements
outside $N\cup\{\sigma\}$ nearest to it, in every combination.  It keeps
the one of at most $n+|N|$ elements with the least $\Phi_r$, unless the
child is given a named set.

(a) Lemma~\ref{lem:vertexopt} with $S=\{2,3\}$, $\{2\}$ and $\varnothing$
gives $\mu_r(\{2,3\},14)=\Phi_r(Z_{2,3})$, $\eta_2(12)=\Phi_r(Z_2)$ and
$\nu_1(12)=\Phi_r(Y_{12})$, and $\Phi_r(Y_{12})<\Phi_r(Z_2)<\Phi_r(Z_{2,3})$.
With $n=12$, $m=2$, $B=\Phi_r(Z_{2,3})$, $Y=Y_{12}$ (so
$T_\varnothing=33$) and the pair $\{2,3\}$ given $Z_{2,3}$, the test checks
...
$V_r(14,3)=1/B$.  With $m=1$, $B=\Phi_r(Z_2)$ and the position $2$ given
$Z_2$, the same check gives $V_r(13,2)\ge1/\Phi_r(Z_2)$, and $S=\{2\}$ gives
equality.  Since $[1,2]\subseteq Z_2$,
$\eta_2(12)\le\nu_3(11)=\mu_r([1,2],13)\le\Phi_r(Z_2)=\eta_2(12)$.  Finally
...
second.  In (b) the certified optimum $Z'_{2,3}$ contains $[1,3]$, as the
certified optima in Proposition~\ref{prop:secondrowexact}(a) and~(d) contain
$[1,\sigma]$; in (a) $Z_{2,3}$ omits $1$.  The identities at
$(\frac43,15,2)$ and $(\frac32,10,3)$ are outside
Theorems~\ref{thm:onepolyrows} and~\ref{thm:longdiag}, which give
$\nu_1(n)\le\nu_k(n)$.

\paragraph{Numerical findings for the third row.}
The following was found in floating point and is not proved.  For
$r\in\{\frac65,\frac54,\frac43\}$ with $2\le n\le18$ and for $r=\frac32$ with
$2\le n\le16$ ($66$ grid points), we computed $\nu_i(n)$ for $i\le3$,
$\eta_\sigma(n)$ for $\sigma<t_2(p_*)$, and $\mu_r(S,n+2)$ for every pair $S$
in the tree of Theorem~\ref{thm:finiterows}(a) with $m=2$, by the line
search of Section~\ref{sec:secondrow}; each $q_N$ is the zero set found for
$N$, and the thresholds were computed in floating point.  So each
$V_r(n+2,3)$ and $V_r(n+1,2)$ below is the finite minimum of the theorem,
with computed rather than certified terms.
\begin{itemize}
\item The prefix identity at $k=3$ failed at $(r,n+2,3)$ for $r=\frac65$,
$n=18$; for $r=\frac54$, $14\le n\le18$; and for $r=\frac43$,
$n\in\{11,12,13,14,18\}$.  The largest relative gap,
$\min_{i\le3}1/\nu_i(n)$ over $V_r(n+2,3)$, was about $1.40$, at
$(\frac54,19,3)$; on this grid the largest gap at $k=2$ was about $1.16$,
at $(\frac54,16,2)$.
\item On a diagonal $n$, the identity failed at $k=3$ but not at $k=2$ only
for $(r,n)=(\frac43,14)$, the case of
Proposition~\ref{prop:thirdrowexact}(b); it failed at $k=2$ but not at
$k=3$ only for $(r,n)=(\frac65,17)$, where the third row was $1/\nu_3(17)$.
\item In the failures with $r=\frac54$, $16\le n\le18$, and with
$r=\frac43$, $12\le n\le14$, the third row was below the second, with the
worst pair $\{2,3\}$; in the other failures it equalled the second row.
\item Where the identity held at $k=3$, its minimum was at $i=3$, with the
pair $\{1,2\}$ worst, or at $i=1$, the third row being the top row.
\item At the $30$ grid points with $\nu_2(n)\ge\nu_1(n)$ the identity held
at $k=2$, now with every position below $t_1(p_*)$ computed, which is
further evidence for the conjecture of Section~\ref{sec:secondrow}.
\end{itemize}

\paragraph{Data availability.}
No datasets were generated or analyzed in this study.  The exact
computations behind Propositions~\ref{prop:secondrowexact} and~\ref{prop:thirdrowexact} are in
\texttt{tests/test\_rows.py} of the accompanying repository.
