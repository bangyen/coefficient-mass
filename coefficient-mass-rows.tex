...
(Proposition~\ref{prop:thirdrowexact}).  The proofs of these two
propositions are computer-assisted: they print the certifying zero sets and
...
satisfies the bound.  The named sets are the certificate: with them the statement
can be rechecked by any exact evaluation of $\Phi_r$ and $G_y$.
...
We conjecture that the prefix identity holds at $(r,n+1,2)$ whenever
$\nu_2(n)\ge\nu_1(n)$.  Whether a worst finite position is always small,
and whether the identity fails for every $1<r<2$ or only near $1$, we do not
know.
...
child is given a named set.  Each threshold $T_N$ is found by evaluating
the sign of $\Delta_{q_N,m-|N|}$ at integers (it does not increase,
Lemma~\ref{lem:farzeros}(a)), and the test checks
$\Delta_{q_N,m-|N|}(T_N)\le0$ exactly.  For the third rows the trees have
$32$, $35$ and $15$ nodes of size one and $126$, $161$ and $33$ leaves in
(a), (b) and (c); at each node and leaf the test checks $\Phi_r\le B$, or
$\Phi_r<B$ where stated, for the set it keeps.
