...
$k$-th entry of the magnitudes $|f_j|$, $j<D$, listed in decreasing order
with multiplicity.  Fix
...
$q(0)=1$ and $q|_S=0$, and recall $\beta_r(n)=V_r(n,1)$.  At
...
displaced-zero tail bound of \cite{coefficient-mass} to arbitrary nodes
in $(0,1)$; there it is proved for nodes $y_i\le\frac12$.  At the root $r$
...
no cutoff.  In the displaced step, with $z=\max Z$ and $u$ as in the
statement (the sum called $V$ there), let $G$ be the sum on all $c$ nodes with
$G|_{\{0\}\cup(Z\setminus\{z\})}=0$ and $G_z=1$, put $\xi=|u_z|$, so that
$w=u-u_zG$, and put $\Gamma_X(z)=\sum_{d\ge z}|X_d|/|X_z|$ for
$X\in\{u,G\}$ and $\Sigma_G=\sum_{1\le d<z}|G_d|$, the notation of the proof of
\cite[Theorem~2.2]{coefficient-mass}.  That proof derives the identity
\cite[eq.~(2)]{coefficient-mass},
$\Tail(u)-\Tail(w)=\xi(2\Gamma_u(z)-\Gamma_G(z)+\Sigma_G)$, and the first
inequality of \cite[eq.~(3)]{coefficient-mass},
$\Gamma_G(z)\le\Gamma_u(z)/(1-y_c)$, from the zero bound of
...
$\Tail(w)<\Tail(u)$.  If $y_c>\frac12$, then $\Sigma_G\ge0$ and
the identity give
\[
  \Tail(w)\le\Tail(u)+\xi\Gamma_u(z)\Bigl(\frac1{1-y_c}-2\Bigr),
\]
where the bracket is positive, and
$\xi\Gamma_u(z)=\sum_{d\ge z}|u_d|\le\Tail(u)$ (if $u_z=0$ then $w=u$), so
$\Tail(w)\le(\frac1{1-y_c}-1)\Tail(u)=\varphi_c\Tail(u)$.  In both cases
$\Tail(w)\le\max(1,\varphi_c)\Tail(u)$.  The set $Z\setminus\{\max Z\}$ has
the same leading run $\ell$ and $\ell+1\le c-1$, so the induction hypothesis
for $u$ on $y_1,\ldots,y_{c-1}$ gives the bound.
\end{proof}
...
$\Tail(w^\varepsilon)\le\max(1,\varphi_c)\Tail(u^\varepsilon)$ of
Theorem~\ref{thm:tailgen} for its sums $w$, $u$ on these nodes; the
...
For finite $Z\subset\N_{>0}$ and an integer $\tau\ge1$ let
\[
...
Only the optimality of $W$ in the single direction $H$ is used.
...
row is attained at a position $\sigma$ whose unique optimum contains all of
...
$\nu_2(n)\ge\nu_1(n)$.  Whether a worst exempted position, when one
exists, is always small, and whether the identity fails for every $1<r<2$
or only near $1$, we do not
know.
...
second.  In (b) the unique optimum $Z_{2,3}(14)$ contains $[1,3]$, as the
unique optima in Proposition~\ref{prop:secondrowexact}(a) and~(d) contain
$[1,\sigma]$; in (a) the unique optimum $Z_{2,3}(12)$ omits $1$.  The identities at
