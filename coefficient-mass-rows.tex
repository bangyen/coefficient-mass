...
rows.  The second row is the minimum of the top row and finitely many terms, one
...
It is not attained in general.  The second row $k=2$ is the minimum of the top
row and finitely many exempted-position terms (Theorem~\ref{thm:secondrow}), and exact
...
$\sigma_0=107$, so $\eta_\sigma(15)\le\Phi_r(Y_{15})$ for $\sigma\ge107$ by
...
\begin{proof}
Both parts rest on one step.  Let $S=\{\sigma_1<\cdots<\sigma_m\}$, put
$N_j=\{\sigma_1,\ldots,\sigma_j\}$, and let $q$ be real with
$\deg q\le n+j-1$, $q(0)=1$, $q|_{N_j}=0$ and
$\Delta_{q,m-j}(\sigma_{j+1})\le0$ for some $j<m$.  The set
...
(b) Let $|S|=m$.  We have $N_0\in\mathcal T$.  If $N_j\in\mathcal T$ and
$\sigma_{j+1}<T_{N_j}$, then $N_{j+1}\in\mathcal T$ when $j\le m-2$, and
$\mu_r(S,n+m)\le B$ by hypothesis when $j=m-1$.  Otherwise some $j<m$ has
$N_j\in\mathcal T$ and $\sigma_{j+1}\ge T_{N_j}$.  Then
...
$N\cup\{\sigma\}$ it tries a few sets: that of $N$ with a nearby element
replaced by $\sigma$ or with $\sigma$ added, and one fixed set of each size
with the elements nearest to the positions of $N\cup\{\sigma\}$ moved there.
It keeps the one with the least $\Phi_r$, unless the child is given a named
set.
...
second.  In (b) the optimum for the worst pair contains $[1,3]$, as the
optima in Proposition~\ref{prop:secondrowexact}(a) and~(d) contain
$[1,\sigma]$; in (a) the optimum $Z_{2,3}$ omits $1$.  The identities at
