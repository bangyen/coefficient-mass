...
each of which can be certified exactly; at $r=\frac54$ and $r=\frac43$ this shows that the
...
\[
  \mu_\sigma(n)=\mu_r(\{\sigma\},n+1)
  =\inf_{p\in\mathcal Q_n}\sum_{s\ge1}\Bigl|1-\frac s\sigma\Bigr||p(s)|\,r^{-s}
  \qquad(\sigma\ge1).
\]
Then $\mu_1(n)=\nu_2(n)$, and Proposition~\ref{prop:rowvalue}(b) gives
$V_r(n+1,2)=\inf_\sigma1/\mu_\sigma(n)$.  Far positions cost no more than
the limit $\nu_1(n)$, so the second row is a minimum over finitely many
positions (Theorem~\ref{thm:secondrow}), and each term can be certified
exactly (Lemma~\ref{lem:vertexopt}).  At $r=\frac54$ and $r=\frac43$
...
rational numbers computable exactly, and the lemma is a finite test.

For $p\in\mathcal Q_n$ and $\sigma\ge1$ put
\[
  M(p)=\sum_{s\ge1}s\,|p(s)|\,r^{-s},\qquad
  T_p(\sigma)=\sum_{s>\sigma}(s-\sigma)\,|p(s)|\,r^{-s}.
\]

\begin{theorem}[The second row is a finite minimum]\label{thm:secondrow}
Let $r>1$ and $n\ge1$.
\begin{enumerate}
\item[(a)] For every $p\in\mathcal Q_n$ and $\sigma\ge1$,
\[
  \mu_\sigma(n)\le A_1(p)-\frac{M(p)-2T_p(\sigma)}\sigma .
\]
\item[(b)] Some $p_*\in\mathcal Q_n$ has $A_1(p_*)=\nu_1(n)$.  Let
$\sigma_*$ be the least $\sigma\ge1$ with $2T_{p_*}(\sigma)\le M(p_*)$.
Then $\mu_\sigma(n)\le\nu_1(n)$ for $\sigma\ge\sigma_*$, with strict
inequality for $\sigma>\sigma_*$, and
\[
  V_r(n+1,2)=\min\Bigl\{\frac1{\nu_1(n)},\
  \min_{1\le\sigma<\sigma_*}\frac1{\mu_\sigma(n)}\Bigr\}.
\]
\item[(c)] The prefix identity~\eqref{eq:prefixid} fails at $(r,n+1,2)$
if and only if $\mu_\sigma(n)>\max\{\nu_1(n),\nu_2(n)\}$ for some
$2\le\sigma<\sigma_*$.
\end{enumerate}
\end{theorem}

\begin{proof}
(a) For real $x$, $|x|=x+2\max\{-x,0\}$; with $x=1-s/\sigma$ this is
$|1-s/\sigma|=1-s/\sigma+2(s/\sigma-1)^+$.  Multiply by $|p(s)|r^{-s}$
and sum; all three series converge absolutely.  This gives
$\Phi_r((1-s/\sigma)p)=A_1(p)-M(p)/\sigma+2T_p(\sigma)/\sigma$, and
$(1-s/\sigma)p$ is admissible for $\mu_\sigma(n)$.

(b) For $n=1$, $\mathcal Q_1=\{1\}$.  For $n\ge2$, the values
$p(1),\ldots,p(n-1)$ are affine coordinates on $\mathcal Q_n$ and
$A_1(p)\ge r^{-(n-1)}\sum_{s<n}|p(s)|$, so the sublevel sets of $A_1$ are
bounded; $A_1$ is convex and finite on $\mathcal Q_n$, hence continuous,
and a minimizer $p_*$ exists.  Since $s\ge1$, $M(p_*)\ge A_1(p_*)>0$, while
$T_{p_*}(\sigma)\le\sum_{s>\sigma}s|p_*(s)|r^{-s}\to0$, so $\sigma_*$
exists.  Moreover
$T_{p_*}(\sigma)-T_{p_*}(\sigma+1)=\sum_{s>\sigma}|p_*(s)|r^{-s}>0$, since
$p_*\ne0$ has finitely many zeros.  So $M(p_*)-2T_{p_*}(\sigma)\ge0$ for
$\sigma\ge\sigma_*$, strictly for $\sigma>\sigma_*$, and (a) with $p=p_*$
gives the first claim.  By Proposition~\ref{prop:rowvalue}(b) and (c),
$V_r(n+1,2)=\inf_\sigma1/\mu_\sigma(n)$ and
...
$1/\nu_2(n)=1/\mu_1(n)$ is one of the terms in (b); if $\sigma_*=1$, then
...
with $2T_p(\sigma_0)\le M(p)$ give $\mu_\sigma(n)\le A_1(p)$ for all
$\sigma\ge\sigma_0$, since $T_p$ decreases.  The next proposition handles
...
$V_r(19,2)=1/\mu_3(18)=1/\Phi_r(Z_3)=1/\nu_4(16)=41.38\ldots$, while
$\mu_2(18)<\nu_1(18)$ and
...
$\mu_2(15)=\Phi_r(Z_*)$, and with $S=\varnothing$ that
$\nu_1(15)=\Phi_r(Y_{15})$, which is less than $\Phi_r(Z_*)$.  The tail
condition $2T_p(\sigma_0)\le M(p)$ holds for $p=r_{Y_{15}}$ at
$\sigma_0=107$, so $\mu_\sigma(15)\le\Phi_r(Y_{15})$ for $\sigma\ge107$ by
the remark after Theorem~\ref{thm:secondrow}.  For every other
$\sigma<107$ an explicit set $Z\ni\sigma$ of $15$ integers has
$\Phi_r(Z)\le\Phi_r(Z_*)$: the set
$Y'_{15}=\{1,3,5,8,12,17,22,28,36,44,54,65,78,94,115\}$ when it contains
$\sigma$, and otherwise $\mathrm{ins}(Y_{15},\sigma)$ or $Y'_{15}$ with
one element replaced by $\sigma$.  Hence
$\sup_\sigma\mu_\sigma(15)=\Phi_r(Z_*)$, and
Proposition~\ref{prop:rowvalue}(b) gives $V_r(16,2)=1/\Phi_r(Z_*)$.  Since
$[1,2]\subseteq Z_*$,
$\mu_2(15)\le\nu_3(14)=\mu_r([1,2],16)\le\Phi_r(Z_*)=\mu_2(15)$.  Finally
...
$\nu_1(n)$, $\nu_2(n)$ and $\mu_\sigma(n)$ by exact line search over integer
...
identity held'' means that no computed $\mu_\sigma(n)$ exceeded
