...
for each nearby exempted position, each attained by a polynomial with integer zeros whose optimality, for
rational $r$, is a finite exact check; at $r=\frac54$ and $r=\frac43$ this shows that the
prefix description fails in general.
\end{abstract}
...
the competitor $(1-\frac s2)(1-\frac s5)$ for $\beta_2(3)$ (Section~\ref{sec:rowvalues}) has
$\sum_{s\ge1}|(1-\frac s2)(1-\frac s5)|2^{-s}=\frac{11}{40}$, so
...
second row for every $n$ and shows that the identity can fail.
...
positions.  Section~\ref{sec:longdiag} shows that a
...
the remark after Theorem~\ref{thm:secondrow}.  For every other
$\sigma<107$ one of the following sets $Z\ni\sigma$ of $15$ integers has
