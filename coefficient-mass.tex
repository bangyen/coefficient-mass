...
F_{m+1}$.  Next let $p\in Z'$.  Then $\widehat F_p=G_p=0$, so
...
$E_d/\widehat F_d\le\sigma$ for $d\ge m+1$.  Next, $\sigma\le G_z/\widehat F_z$, with equality
when $z=m+1$.  So let $z>m+1$, put $\tau=G_z/\widehat F_z>0$, and consider
the $c$-node sum
\[
  \Theta_d=G_d-\tau\widehat F_d .
\]
It vanishes on $Z'$ and at $z$, which is $c-1$ points, and
$\Theta_0=-\tau\widehat F_0=-\tau\eta\ne0$, so by the zero bound $\Theta$
changes sign exactly at those points.  The $c-2$ points of $Z'$ lie in
$(0,m+1)$ and $z$ does not, so $\sgn\Theta_{m+1}=(-1)^{c}\sgn\Theta_0
=-(-1)^{c}\eta$.  And $\eta=(-1)^{c}$, since $F_0=1$ and $F$ flips at each of
