...
$\ell^1$ tails of normalized exponential sums in the reciprocal roots
$1/r_i$ with prescribed positive-integer zeros: deleting the largest zero
together with the largest node, the reciprocal of the smallest root,
strictly increases the tail, and iterating gives a product bound over
...
Write $b_k(F)$ for the $k$-th largest of the $D$ numbers $|f_j|$, $j<D$,
counted with repetition, zeros included.  Theorem~\ref{thm:order} and
...
Sections~2 and~3 through Corollary~\ref{cor:mass} are formalized in Lean~4
with Mathlib, in the directory \texttt{CoefficientMass} of the repository
\href{https://github.com/bangyen/coefficient-mass}{bangyen/coefficient-mass}.
The formal proofs follow the ones given here.  Two clauses are not
formalized: the equality case of Theorem~\ref{thm:tail}, and the relaxation
of $r_1\ge2$ to $r_1>1$ in the first row of Lemma~\ref{lem:slack} and
...
node, which keeps the hypothesis, and apply the mean value theorem on each
$[t_{k-1},t_k]$: the derivative, a sum on the other $r-1$ nodes, satisfies
...
largest node $y_c=1/\rho_c$.  The hard step is the following comparison of a
correction sum with its value just past the prescribed zeros, proved by
weak alternation.
...
The bound \eqref{eq:jensen} is useful only while $(1-R)^{-k}$ has not
overtaken $(2R)^{L-k+1}$, that is, for $k$ up to a constant multiple of
...
$P_1=(x-3)(x-\tfrac72)$ exceeds $P_1(1)=5$ in absolute value, so there the row $k=2$ is not an infimum.
...
So row $k$ at $L$ follows, for every $D$, from the statement $(\ast_n)$ with
...
$K=\{1\}$, $\{2\}$, $\{3,c_2\}$ for $j=0,1,2$, fill the next $n-1$ holes
after $K$, and put
\[
...
\emph{The base.}  For $j=0$, $Y=\{2,\ldots,N\}$ with $N=c_n\ge n$, and
\eqref{eq:onehole} gives $\Phi(Y)=1/N\le1/n$.  For $j=1$,
$Y=\{1,\ldots,N\}\setminus\{2\}$ with $N=c_n\ge n+1$, and
$\Phi(Y)=1/(N-1)\le1/n$.  For $j=2$ put $b=c_2\ge4$; the $n-1$ filled holes
...
There remains $(\ast_n)$ for $A\supseteq\{1,2,3\}$, that is, a leading run
...
consecutive block $\{j+1,\ldots,2j+1\}$, and the tail beyond $N$ by a Beta
integral.  Throughout, $j\ge0$, $C=\{c_1<\cdots<c_{j+1}\}\subset\N_{>0}$ with
$c_1=j+1$, $m=c_{j+1}$, $q\ge0$, $N=m+q$, $Y=\{1,\ldots,N\}\setminus C$ and
$\psi=r_Y$.  Then $m\ge2j+1$, and by the proof of Lemma~\ref{lem:holeint}
$\Phi(Y)=H(C,q)+T(C,q)$, where
\[
  H(C,q)=\sum_{c\in C}|\psi(c)|\,2^{-c},\qquad
  T(C,q)=\sum_{s>N}|\psi(s)|\,2^{-s}
        =\sum_{s>N}\binom{s-1}N2^{-s}\prod_{c\in C}\frac c{s-c}.
\]

\begin{lemma}[Values at the kept holes]\label{lem:holevals}
$H(C,q)\le\dfrac1{2(q+1)}$.
\end{lemma}
...
\emph{Factors below $c$.}  $Y\cap[1,c)$ consists of $\{1,\ldots,j\}$ and a
set $E\subseteq\{j+2,\ldots,c-1\}$ that avoids the $i-2$ holes
$c_2,\ldots,c_{i-1}$, so $|E|=a:=c-j-i$.  The factor $(c-p)/p$ is
...
\[
  \frac{U_i(c+1)}{U_i(c)}
  =\frac{c-j-1}{2(c-j)}\cdot\frac c{c-i+2}\cdot\frac{c+i'+1}{c+i'+q+1}
  \le\frac c{2(c-i+2)}\le1,
\]
the last step because $c\ge j+i\ge2i-1$ (as $i\le j+1$).  Since
$c_i\ge j+i$, we get $2^{-c_i}|\psi(c_i)|\le U_i(j+i)$ for every $i$; for $i=1$
this is the bound itself.

\emph{The block.}  For $C^\circ=\{j+1,\ldots,2j+1\}$ with the same $q$, all
three bounds are equalities: $E=\varnothing$, the holes above
$c_i^\circ=j+i$ are $j+i+1,\ldots,2j+1$, and $N^\circ=2j+1+q=c_i^\circ+i'+q$.
So $2^{-c_i}|\psi(c_i)|\le2^{-c_i^\circ}|\psi^\circ(c_i^\circ)|$, where $\psi^\circ=r_{Y^\circ}$ and $Y^\circ=\{1,\ldots,N^\circ\}\setminus C^\circ$, term by term,
and $H(C,q)\le H(C^\circ,q)$.  By the proof of Lemma~\ref{lem:holeint},
$H(C^\circ,q)=\int_0^1v^q\sum_{c\in C^\circ}|\pi_c|2^{-c}v^{2j+1-c}(1-v)^{c-1}\,dv$
with $|\pi_{j+i}|=(2j+1)!/\bigl(j!\,(i-1)!\,(j+1-i)!\bigr)$, and the binomial
theorem gives
\[
  \sum_{i=1}^{j+1}|\pi_{j+i}|2^{-j-i}v^{j+1-i}(1-v)^{j+i-1}
...
gives $H(C^\circ,q)\le\frac12\int_0^1v^q\,dv=\frac1{2(q+1)}$.
\end{proof}

\begin{lemma}[The tail beyond the kept holes]\label{lem:holetail}
If $j\ge1$ and $q\ge1$, then $T(C,q)\le\dfrac{j+1}{e\,j\,(q+1)}$.
\end{lemma}
...
\]
Apply $1/\binom\nu j=(\nu+1)\int_0^1t^j(1-t)^{\nu-j}\,dt$ with $\nu=s-m-1+j$, and
note that $\nu+1=s-m+j\le s-j-1$ because $m\ge2j+1$:
\[
...
\[
  T(C,q)\le(j+1)\binom mj\int_0^1t^j
...
\[
  (q+1)\,T(C,q)\le\frac{j+1}e\binom mj\int_0^\infty t^{j-1}(1+t)^{-m-1}\,dt
...
\[
  N=c_{j+n},\qquad Y=\{1,\ldots,N\}\setminus C,\qquad Z=Y\cup A .
\]
Then $Y\supseteq A\cap[1,N]$, the elements of $Z\setminus A$ are the $n-1$
...
$\Phi(Z)\le\Phi(Y)$.  Put $m=c_{j+1}$ and $q=N-m\ge n-1\ge1$.  By
Lemmas~\ref{lem:holevals} and~\ref{lem:holetail},
\[
  \Phi(Y)=H(C,q)+T(C,q)\le\frac1{q+1}\Bigl(\frac12+\frac{j+1}{e\,j}\Bigr)
