...
\end{equation}
Let $\widehat V_d=\eta V_d>0$ for $d>m$ and
...
$c_1=j+1$, $m=c_{j+1}$, $k\ge0$, $N=m+k$, $Y=\{1,\ldots,N\}\setminus C$ and
$\psi=r_Y$.  Then $m\ge2j+1$, and by the proof of Lemma~\ref{lem:holeint}
$\Phi(Y)=H(C,k)+T(C,k)$, where
\[
  H(C,k)=\sum_{c\in C}|\psi(c)|\,2^{-c},\qquad
  T(C,k)=\sum_{s>N}|\psi(s)|\,2^{-s}
...
$m\ge c+i'$, we have $N\ge c+i'+k$, and hence
\[
...
$c_i^\circ=j+i$ are $j+i+1,\ldots,2j+1$, and $N^\circ=2j+1+k=c_i^\circ+i'+k$.
So $2^{-c_i}|r_Y(c_i)|\le2^{-c_i^\circ}|r_{Y^\circ}(c_i^\circ)|$ term by term,
and $H(C,k)\le H(C^\circ,k)$.  By the proof of Lemma~\ref{lem:holeint},
$H(C^\circ,k)=\int_0^1v^k\sum_{c\in C^\circ}|\pi_c|2^{-c}v^{2j+1-c}(1-v)^{c-1}\,dv$
...
$\Phi(Z)\le\Phi(Y)$.  Put $m=c_{j+1}$ and $k=N-m\ge n-1\ge1$.  By
Lemmas~\ref{lem:holevals} and~\ref{lem:holetail},
\[
  \Phi(Y)=H(C,k)+T(C,k)\le\frac1{k+1}\Bigl(\frac12+\frac{j+1}{e\,j}\Bigr)
