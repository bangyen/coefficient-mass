...
- [complex](papers/coefficient-mass-complex.tex): the least fixed
  separation for the mass, between `2 + 1/18915` and 81 (at most 12 with
  two roots above the gap), and whether 9 suffices; the least separation
  for the rows of `cor:annuli`, between `n_a` and `3 rho_(n_a) < 7 n_a`
  (`sec:annuli`).
...
- **Coefficient mass at complex roots.**  Purely imaginary pairs are Closed
  ...
  `3 rho_(n_a) < 7 n_a`.  Closed for the mass at a fixed separation: every
  ...
  (whether 9 suffices; for `S = 2`, `K_2 = 2` it is at most 12), and the
  least separation for the rows, between `n_a` and `3 rho_(n_a) < 7 n_a`.
