...
the coefficients of $P_uM$.  For roots at least $2$, repeated roots
...
\]
satisfy $\xi_0=1$ and $\xi_z=0$ for a set $Z$ of $m-1$ positive integers with
leading run $f$.  Then
\[
...
$\xi^{(\varepsilon)}_z=0$ for $z\in Z$; existence and uniqueness are the
...
$v=v^*+t\delta$ with $t=\tau\sigma\sgn\bigl(S_j(R)/R(1)\bigr)$ keeps the sign
...
\[
  ...
  =\Tail(v^*)-\tau\Bigl(\Bigl|\frac{S_j(R)}{R(1)}\Bigr|-1\Bigr)
  <\Tail(v^*).
\]
\end{proof}
...
$\bigl((x\,d/dx)^aF\bigr)(r_i)=\sum_{j\le D}f_jj^ar_i^j=0$ for $a<m_i$.  Minimising the
...
Lemma~\ref{lem:delete} fails at these roots, where $y_1=\tfrac{10}{11}$:
