...
finitely many single-placement values; this gives the infimum
...
of the escaping minimiser has an exact cost identity, which makes every
...
that below $2$ it need not be.  Section~\ref{sec:belowtwo} treats roots
below $2$: attainment of $T$, the reduction for one exempt coefficient, and
the exact examples.  Section~\ref{sec:scope} closes with a remark
...
(Theorem~\ref{thm:family}); at $(r,5,7)$ it fails for every $r<2$
...
In particular, if $T>T_1$ then $T=\tau(X)$ for a placement $X$.
\end{proposition}
...
$2\sum_{d=1}^N|H_d|\ge\Tail(H)$.  Then $\tau(\{x\})<\tau^*$ for every
$x\ge N$, and
\[
...
multiple attains the infimum; the multiples of degree at most $10$ have
$b_2\ge41.2$.
...
which settles $(\tfrac{11}{10},3,5,7)$ and the family $(r,3,5,7)$
