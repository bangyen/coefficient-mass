...
of~\cite[Theorem~2.2]{coefficient-mass}, which we restate.  Let
$0<y_1<\cdots<y_c\le1/2$, let $Z$ be a set of $c-1$ positive integers,
$z=\max Z$ and $Z'=Z\setminus\{z\}$, and let $w$ be the sum on all $c$
...
\]
using only the zero bound, the distinctness of the nodes and, in the last
inequality, $y_c\le1/2$, not the induction hypothesis of that proof.
Together they give
\[
  \Tail(w)\le\Tail(V)-\mu\Sigma_G ,
\]
and the same step shows $\Sigma_G>0$ when $Z\ne\{1,\ldots,c-1\}$.
...
value $t$, and $u$ exempt coefficients at the bottom.  After division by
$r_i^D$ the exempt coefficients matter only at the $u$ smallest roots,
...
cross coefficients are $O((r_u/r_{u+1})^D)\to0$.  In the limiting matrix
...
is nonsingular, hence the limiting matrix is, and since matrix inversion is
...
block---and $r_u<r_{u+1}$ is what kills the cross terms.
\end{proof}
...
fixed and either $E_\nu=\varnothing$, and we put $j=0$, or $|E_\nu|=j\ge1$
and $\min E_\nu\to\infty$.  A full certificate on $y_{j+1},\ldots,y_L$ whose
zero set contains $X_0$ exists, since $|X_0|=u-j\le L-j-1$; call it
$v^\infty$.  If $E_\nu=\varnothing$, then $T=\tau(X_0)\le\Tail(v^\infty)$.
Otherwise Proposition~\ref{prop:farrep}(c), with $k=j$, gives certificates
for $X_\nu$ with tails tending to $\Tail(v^\infty)$, so
$T\le\Tail(v^\infty)$.  Either way $T<\infty$.
...
with $v^\infty$ a full certificate on $\mathcal H_j$ whose zero set contains
$X_0$, which exists by unisolvence, and Proposition~\ref{prop:farrep}(c),
...
\end{align*}
computed by exact rational arithmetic in
...
are in \texttt{tests/test\_attainment.py} of the repository
