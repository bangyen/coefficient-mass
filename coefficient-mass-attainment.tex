...
the exempt coefficients escape to the bottom costs $\tau^*$, the least
$\ell^1$ tail of a certificate on the surviving roots, and identify
...
\[
  \Tail(\xi)=\sum_{d\ge1}|\xi_d|\le\prod_{i=1}^{f+1}\frac{y_i}{1-y_i},
\]
the product over the $f+1$ smallest nodes, with equality exactly at $Z=\{1,\ldots,c-1\}$.  Lemma~2.1 there evaluates
...
$x^m,\ldots,x^{m-j}$ (so $S_0(P_u)=1$), so that $S_j(P_u)=P_u(1)=(-1)^m\prod_{i>u}(r_i-1)$ once $j\ge m$.  All
...
$T$ is the supremum, over placements $X$ of the $u$ exempt coefficients, of
the least tail $\sum_{d\ge1}|v_d|$ of an exponential sum
$v_d=\sum_i\lambda_ir_i^{-d}$ with $v_0=1$ and $v|_X=0$
...
leading run $f=\max\{0\le k\le m-1:\{1,\ldots,k\}\subseteq Z\}$.  Then
\[
  \Tail(\xi)\le\prod_{j=1}^{f+1}\frac{y_{(j)}}{1-y_{(j)}}.
\]
\end{proposition}
...
$z_{i,j}=y_i\exp(-j\varepsilon)$.  For $0<\varepsilon<\varepsilon_0$ the $m$
...
run $f$.  Keep the perturbed nodes $z_{i,j}=y_i\exp(-j\varepsilon)$ of the
...
\]
Theorem~2.2 on the $m-1$ nodes of $V^{(\varepsilon)}$ bounds
$\Tail(V^{(\varepsilon)})$ by the product over their $f+1\le m-1$ smallest
members, which are the $f+1$ smallest of all $m$ perturbed nodes because the
one removed is the largest.  So
$\Tail(\xi^{(\varepsilon)})\le\prod_{j\le f+1}y_{(j)}(\varepsilon)/(1-y_{(j)}(\varepsilon))
...
\[
  ...
  \le\prod_{j=1}^{f+1}\frac{y_{(j)}}{1-y_{(j)}}-|V^{(0)}_z|\,\Sigma_{G^{(0)}}
...
$|S_m|$ is exactly \eqref{eq:order}: the construction reproduces the bound because the sum of the
...
\begin{proof}
Theorem~3.2 gives $\ge$.  For the other direction put $m=L-u$ and write, for $D\ge L$,
...
values $\sigma_i=r_i^{-D}C(r_i)$ ($i\le u$), a Vandermonde change of variables.  The
...
$1/2\ge y_1>\cdots>y_L>0$ (the reverse of the order in Theorem~2.2), and call an exponential sum
...
partial sums $S_j(R)$, and $v^*$ the certificate on the nodes $1/\rho_i$ with
zero set $\{1,\ldots,n-1\}$, so that $\Tail(v^*)=1/|R(1)|$ by Lemma~2.1.  If
...
$N(x)=\sum_{t=j}^{n-1}\pi_{t-j}x^t$.  Since $\Pi$ is $R$ times the constant
...
\[
  ...
  =-\frac{S_j(R)}{R(1)} .
\]
By Lemma~2.1, $v^*_d$ has the constant sign $(-1)^{n+1}$ for $d\ge n$
and $|v^*_d|\ge\bigl(\prod_iy_i\bigr)y_{\max}^{\,d-n}$, while
$|\delta_d|\le C_\delta y_{\max}^{\,d}$ for a constant $C_\delta$; so for
$\kappa>0$ small enough $v=v^*+t\delta$ with
...
\[
  \Tail(v)=\kappa+(-1)^{n+1}\sum_{d\ge n}\bigl(v^*_d+t\delta_d\bigr)
...
Hermite space of $R$.  The proof of Lemma~2.1 is a generating-function
...
$\Tail(v^*)=1/|R(1)|$; the sum $\delta$ is given explicitly by its generating
...
$|\delta_d|\le C_\delta(1+d)^{\mu-1}y_{\max}^{\,d}$, so $|\delta_d/v^*_d|$ is still
...
If $X_0=\varnothing$, $\mathcal H_u$ is the Hermite space of the roots of $P_u$, and
...
If $X_0\ne\varnothing$, take the certificate of Lemma~3.1's proof on $\mathcal H_k$:
...
node, and the payment is the deletion step of Theorem~2.2 read backwards.

\begin{lemma}[The deletion step]\label{lem:delete}
Let $c\ge2$, let $\tfrac12\ge y_1>y_2>\cdots>y_c>0$, let $Z$ be a set of $c-1$
positive integers with $z=\max Z$, let $w$ be the full certificate on
$y_1,\ldots,y_c$ with zero set $Z$, and let $F$ be the full certificate on
$y_2,\ldots,y_c$ with zero set $Z\setminus\{z\}$.  Then
$\Tail(w)\le\Tail(F)$.
\end{lemma}

\begin{proof}
If $Z=\{1,\ldots,c-1\}$, Lemma~2.1 evaluates both tails as products, which
differ by the factor $y_1/(1-y_1)\le1$.  Otherwise $Z$ has a displaced zero,
and the displaced step of the proof of Theorem~2.2 (whose nodes are listed in
increasing order) deletes the largest prescribed zero together with the
largest node and proves $\Tail(w)<\Tail(F)$ for nodes at most $\tfrac12$; that
...
consecutive zero set $\{1,2,3\}$ and, by Lemma~2.1, the tail
...
as above.  Let $F$ and $F_j$ be the full certificates on
...
Each of $F$, $F_j$, $w$ is full, so by the zero bound it has exactly its
...
Hence $F(d)=\varphi(d)|F(d)|$.  For an integer $d$,
...
\end{equation}
In words: the $F_j$ keep the signs of $F$ at every integer, and $w$ keeps
them up to $g$ and reverses them above.

\emph{The combination.}  At a run start $s_i$ we have $F_j(s_i)=0$ for
$j\ne i$, as $s_i\in Z_j$; $F_i(s_i)\ne0$, as $s_i\notin Z_i$; and
...
$\sgn\omega=(-1)^{L-k-1}$.  The sum $G=F-q$ on $y_{k+1},\ldots,y_L$ has weight
...
$\alpha=\sum_j\lambda_j|F_j(d)|$ and $\beta=\mu|w(d)|$; by \eqref{eq:runsigns}
$q(d)=\varphi(d)(\alpha-\beta)$, so $\gamma_d=\alpha+\beta-|\alpha-\beta|$ and
\[
  \delta_d-\gamma_d=|F(d)|-\alpha-\beta=\varphi(d)G(d)-2\mu|w(d)| ,
\]
...
and $A=\{7\}$ its right side exceeds $\Tail(F)$.
...
$y_{k+2},\ldots,y_L$, $\tau_{k+1}(X')$ is attained by a full certificate $F$
...
$\tau_k(X)\le\Tail(v^+)\le\Tail(F)=\tau_{k+1}(X')$.
...
$\tau_{k+1}(X')=\Tail(F)$.  In \eqref{eq:runchord} every $\Tail(F_j)$ is
therefore at least $\Tail(F)$, so
$\Tail(F)\ge(1-\mu)\Tail(F)+\mu\Tail(w)$ with $\mu>0$, that is
$\Tail(w)\le\Tail(F)$.  The zero set $B_0\cup\{g\}\cup(A+1)$ of $w$ contains
...
not certified this.  Numerically the cutoff does not appear to be necessary: at
$(\tfrac32,3,5,7)$ a numerical search finds every placement up to distance
$12$ strictly below $\tau^*$.
