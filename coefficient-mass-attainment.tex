...
$W=B_0\cup\{g\}\cup(A+1)$.  Then there are $\lambda_1,\ldots,\lambda_p,\mu>0$
...
with the $L-k-1$ zeros $W$; and since $y_{k+1}$ is the largest node and $w$
has the sign $(-1)^{|W|}=(-1)^{L-k-1}$ past $\max W$,
...
$g\in R_t$.  At a gap start $h=r+1$ with $r\in S$, $r<g$ and $h\notin S$, we
have $G(h-1)=0$ and $h\notin W$, so $E(h)=2\mu w(h)\ne0$, of sign
...
In all $E$ has at least $|S|-2+|A|=|B_0|+|A|=L-k-2$ zeros, of which at least
...
order of the indices, of a value $\rho$ of multiplicity $e$, put
$r_{i,\varepsilon}=\rho e^{j\varepsilon}$ ($0\le j<e$).  For small
...
$\Tail(v)=\tau^*$; write
$v_d=\sum_\rho\sum_{a<e'_\rho}c_{\rho,a}d^a\rho^{-d}$, where $e'_\rho$ is the
number of indices $i>u$ with $r_i=\rho$.  Those indices carry the $e'_\rho$
largest copies of $\rho$, so the surviving perturbed nodes at $\rho$ are
$z_{\rho,\varepsilon}e^{-j\varepsilon}$, $0\le j<e'_\rho$, with
$z_{\rho,\varepsilon}=\rho^{-1}e^{-(e_\rho-e'_\rho)\varepsilon}$.  Put
\[
  v^\varepsilon_d=\sum_\rho\sum_{a<e'_\rho}c_{\rho,a}\,
  z_{\rho,\varepsilon}^{\,d}\Bigl(\frac{1-e^{-\varepsilon d}}{\varepsilon}\Bigr)^a .
\]
...
$0\le(1-e^{-\varepsilon d})/\varepsilon\le d$, every $|v^\varepsilon_d|$ is
...
not certified this.  The cutoff is sufficient but not necessary: at
