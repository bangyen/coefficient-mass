...
$1/\tau^*$; below $2$ it can be strictly smaller.  No infimum of $b_{u+1}$
...
that below $2$ it need not be.  Section~\ref{sec:scope} closes with a remark
...
L'H\^opital's rule then gives $\varphi_{j+1}/\varphi_j$ the same limit, so it is $0$:
...
$\widehat Z=B_0\cup\{g\}\cup(A+1)$.  Then there are $\lambda_1,\ldots,\lambda_p,\mu>0$
with $\sum_j\lambda_j+\mu=1$ such that
...
$w(s_i)\ne0$, as $s_i>g$ and $s_i-1\notin A$ keep $s_i$ out of $\widehat Z$.  Put
$\rho_i=|w(s_i)|/|V_i(s_i)|$, $\mu=1/(1+\sum_i\rho_i)$, $\lambda_i=\mu\rho_i$
and
\[
...
\eqref{eq:runchord} can fail: with the nodes
...
tail $\tfrac{96935}{3398808}=0.02852\ldots>\tfrac{31}{1704}$.  Numerically
$X=\{5\}$ is the worst placement and $\{1,3,5\}$ its minimising zero set,
which would make the infimum $\tfrac{3398808}{96935}=35.06\ldots$; we have
not certified this.  Numerically the cutoff does not seem necessary at every
root set: at $(\tfrac32,3,5,7)$ every placement up to distance $12$ stays
strictly below $\tau^*$.
...
gives $1/T$ with $T\ge\tau^*$, and Proposition~\ref{prop:belowtwo} shows
that $T>\tau^*$ can occur.  Which placement is then the worst, whether $T$
is a finite computation, and for which roots below $2$ the escaping
placement stays extremal (numerically it does at $(\tfrac32,3,5,7)$) are
left open.
