...
bottom costs $\tau^*$, the least $\ell^1$ tail of an exponential sum on the
...
finitely many single-placement values; with an explicit multiplier this gives the infimum
...
$1<r<2$.  No infimum of $b_{u+1}$
over monic multiples is attained.  Along the way we extend the underlying tail comparison to
...
bottom; at repeated roots sufficiency comes from duality instead.
...
(Proposition~\ref{prop:belowtwo}).  No infimum of $b_{u+1}$ over monic
multiples is attained (Theorem~\ref{thm:noattain}).
...
$48(r-1)(15r+71)/(49r-34)$ (Theorem~\ref{thm:family}).  Along $(r,5,7)$ it
...
$z_{i,j}=y_i\exp(-j\varepsilon)$.  For all sufficiently small $\varepsilon>0$ the $m$
...
sum on the nodes $z_{i,j}$ with $\xi^{(\varepsilon)}_0=1$ and
...
run $\ell$.  Keep the perturbed nodes $z_{i,j}=y_i\exp(-j\varepsilon)$ of the
...
displaced step of the proof of~\cite[Theorem~2.2]{coefficient-mass} once at these $m$ distinct nodes:
...
$G^{(\varepsilon)}_z=1$, and $\mu_\varepsilon=|V^{(\varepsilon)}_z|$.  The
splitting identity and the correction-ratio bound
$\Gamma_G(z)\le2\Gamma_V(z)$ there (all nodes are at most $1/2$) give
\[
  \Tail(\xi^{(\varepsilon)})
...
coefficients grow like $r_u^D$ and so survive division by $r_i^D$ only at
...
values $\sigma_i=r_i^{-D}C(r_i)$ ($i\le u$), a Vandermonde change of variables.  As
$D\to\infty$ the system tends to a block triangular one, whose surviving rows
say that the monic $(x-1)\bar A+\bar t$ of degree $m$ vanishes at the $m$
surviving roots, where $\bar t$ and $\bar a_k$ are the limits of $t$ and
$a_k$ and $\bar A=x^{m-1}+\bar a_1x^{m-2}+\cdots+\bar a_{m-1}$.  So
$(x-1)\bar A+\bar t=P_u$, and as $D\to\infty$
\[
  t\to P_u(1),\qquad
  a_k\to S_k\quad(1\le k<m),\qquad
  \sigma_i\to-\frac{r_i^{1-m}P_u(r_i)}{r_i-1}\ne0 ,
\]
since $\bar A=(P_u-P_u(1))/(x-1)$ has as coefficients the prefix
$S_1,\ldots,S_{m-1}$ of \eqref{eq:wgen}.  The perturbation is
$O((r_u/r_{u+1})^D)$, read as $O(r_1^{-D})$ at $u=0$, and every exempt
...
sign past its largest zero.  By \eqref{eq:weak} every monic multiple $F$ has
...
best for the multiple.  We call $\tau(X)$ the \emph{cost} of $X$.  The first result says that it is exactly that, and that exempt
...
real zero, changes sign at each prescribed zero and keeps one sign past its
largest zero.  For finite $X\subset\N_{>0}$ put
\[
...
differ by the factor $y_1/(1-y_1)\le1$.  Otherwise $Z$ has a displaced zero,
and the displaced step of the proof of~\cite[Theorem~2.2]{coefficient-mass} (whose nodes are listed in
increasing order) deletes the largest prescribed zero together with the
largest node and proves $\Tail(w)<\Tail(V)$ for nodes at most $\tfrac12$; that
step does not use the induction hypothesis of the proof.
\end{proof}
...
Each of $V$, $V_j$, $w$ is full, so by the zero bound it has exactly its
prescribed zeros, all simple, and it is positive at $0$; its sign at an
...
and $A=\{7\}$ its right side exceeds $\Tail(V)$.
...
$(\tfrac13,\tfrac15,\tfrac17)$ with zero set $\{1,3\}$ has tail
$\tfrac{31}{1704}$, and the multiplier $M=x+\tfrac9{62}$ has
$\max_{j\ge1}|S_j(P_1M)|=\tfrac{1704}{31}$, so by \eqref{eq:dualpair} both
attain $\tau^*=\tfrac{31}{1704}$ and Theorem~\ref{thm:extremal} gives
\[
  \inf_Qb_2\bigl(Q(x-2)(x-3)(x-5)(x-7)\bigr)=\frac{1704}{31}=54.967\ldots,
\]
strictly between the bound $48$ of \eqref{eq:order} and the value $57$ of the
construction of Proposition~\ref{prop:partial}.
...
gives the full certificate on all four nodes with zero set $\{1,3,5\}$ and
tail $\tfrac{96935}{3398808}=0.02852\ldots>\tfrac{31}{1704}$.
...
coordinate of $X_\nu$, listed in increasing order, is bounded or not along
any subsequence, so after passing to one, $X_\nu=X_0\cup E_\nu$ with $X_0$
fixed and either $E_\nu=\varnothing$ or $|E_\nu|=j\ge1$ and
...
$\{1,4,9\}$ and $\{1,3,x\}$ have tails $0.02082\ldots$, $0.02562\ldots$,
$0.02151\ldots$, $0.01914\ldots$, $0.01842\ldots$, $0.01823\ldots$ and
$0.01818\ldots$, all below $\theta=0.02852\ldots$ (each is an exact
rational, a finite head plus a geometric remainder past the largest
zero).  So $T=\theta$, attained only at $\{5\}$, and
Theorem~\ref{thm:value} gives the infimum.
\end{proof}
...
The window $g<x<N$ is needed here: the aligned lift costs more than $\tau^*$
at $x=4,\ldots,9$ and less at $x=10$.  By Theorem~\ref{thm:noattain} no
multiple attains the infimum.  For each degree $D\le10$ and each exempt
position, the least $\max|f_j|$ over the remaining positions $j<D$ of monic
multiples of degree $D$ is a linear program, which we solved exactly over
$\mathbb Q$ (\texttt{tests/test\_attainment.py}); every optimum is at least
$41.2$, so the multiples of degree at most $10$ have $b_2\ge41.2$.
...
\[
  ...
  \end{cases}
\]
over monic $Q$, and for $\tfrac{29}{20}\le r<r_c$ the worst placement is
$\{4\}$ and no other.
\end{theorem}
...
\emph{Lower bounds.}  For $M=x+m$ the partial sums of $P_rM$ are
$S_1=m-r-14$, $S_2=14r+57-m(r+14)$, $S_3=m(14r+57)-57r-48$,
$S_j=(P_rM)(1)=48(m+1)(r-1)$ for $j\ge5$, and $S_4$, which we exempt.
Lemma~\ref{lem:exemptpair} with $X=\{4\}$ gives
$T\ge\tau(\{4\})\ge1/\max_{j\ne4}|S_j|$.  At $m=\tfrac32$ each $S_j$ is
...
\]
and $|S_1|,|S_3|\le\beta(r)$ amounts to $671r^2+2002r-2827\ge0$,
...
There $\tau(\{4\})\ge1/\beta(r)$, and $1/\beta(r)-\tau^*$ has the sign of
$1988+7r-930r^2$, positive exactly for $r<r_c$.
...
\[
  ...
  \frac{4267r^2+5070r-5871}{48(r-1)(3466r^2+7455r+11025)},
\]
the second at most $\tau^*$ exactly when
$c_2(r)=214892r^3-55639r^2-138630r-266709\ge0$.  For the $H$ of
Theorem~\ref{thm:halfmass}, $2\sum_{d\le5}|H_d|-\Tail(H)$ has the sign of
$c_5(r)=206070r^3-67123r^2-161312r-238560$ on $(1,3)$.  Both cubics
increase on $r\ge1$ (their derivatives are positive there) and are positive
...
$\tau^*$.  Theorem~\ref{thm:value} turns these into the stated infima.
\end{proof}
...
above $Z^*$, where the conclusion of Lemma~\ref{lem:delete} fails.  Numerically (floating
point, placements up to $24$ and zero sets inside $[1,26]$, $r$ on a grid of
step $\tfrac1{20}$), the worst placement for $r<\tfrac{29}{20}$ is $\{4\}$
down to $r=1.15$ and $\{5\}$ at $r=1.05,1.1$.  The cutoff cannot be lowered
uniformly, however.
...
field $\mathbb K$ with all roots in $\mathbb K^\times$ grows exponentially in $n$~\cite{tromp}.  The
