...
over $\mathbb Q(r)$ respectively, which the proofs state and the
accompanying tests carry out.  Along $(r,5,7)$ it
...
$0.01818\ldots$, all below $\theta=0.02852\ldots$.  So $T=\theta$, attained
...
past $\max Z$ by the zero bound, so the last sum is again geometric), and
checks $\Tail(w)<\theta$ exactly.  The partial sums of $PM$ and the tail of
...
multiples of degree $D$ is a linear program in the $D-4$ free coefficients
of $Q$ and a bound $t$.  By exact rational arithmetic, in
\texttt{test\_value\_eleven\_tenths}, a simplex method over $\mathbb Q$
solves each of these finitely many programs, and each optimum is certified
by a feasible point and a dual solution of the same value (weak duality), so
the optima are exact; every one is at least $\tfrac{206}5$, so the multiples
of degree at most $10$ have $b_2\ge\tfrac{206}5=41.2$.
...
computations behind Theorems~\ref{thm:elevenvalue} and~\ref{thm:family}
are in \texttt{tests/test\_attainment.py} of the accompanying repository.
