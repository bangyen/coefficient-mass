...
survives for complex roots.  For fixed $m$, all $m$-th roots of real
numbers $s_j$ of one sign, in particular purely imaginary pairs $\pm ic$
(the square roots of $-c^2$), reduce exactly to real roots: the infimum of
every $b_k$ and of $\Lambda$ over monic multiples is the same as at the real
roots $|s_j|$ ($c^2$ for $\pm ic$).  Without such structure only the
first row survives uniformly in the angles: $b_1(F)\ge|f_D|\prod_j(|\alpha_j|-1)$ at arbitrary roots
of modulus greater than one, while $b_2$ and every superlinear mass bound
fail to hold uniformly over pairs within any fixed angle of the imaginary axis.  Rows and mass
return, up to constants, when the moduli are separated by a factor $9$, by
an Archimedean Newton polygon argument.  With $K_s$ roots of modulus in
$[T_s,U_s]$ for $s=1,\ldots,S$, where $T_1>3$ and
$T_s>3\cdot9^{K_s+\cdots+K_S}U_{s-1}$ (whether a fixed factor suffices is
open), monic multiples have mass at least $\sum_ssK_s\log(T_s/2)-O(S)$,
attained to leading order for suitable roots as $\min_sT_s\to\infty$: a root is charged once per annulus at or below it, and the
cross terms $K_uK_s\log T_s$ of the real bound fail.  Real roots of both
signs charge one
...
half-width $\delta<\pi/2$, multiples of degree $O(K)$ with leading
coefficient of modulus at least $1$ have mass at least
$(\frac12-o(1))K^2\log R$ if $\delta K\to0$ and at least $(\frac\pi\delta-o(1))K\log R$
at fixed $\delta$, as $K,R\to\infty$, with both constants attained; at
distinct Gaussian-integer roots in the upper half-plane and fixed $\delta$ the degree restriction is removed
...
zeros included ($b_k(F)=0$ for $k>D$).  For real roots
...
greater than one on the condition that their determinant sequence
eventually has constant sign~\cite[p.~333]{dj}, a condition removed
in~\cite[Lemma~2.1]{coefficient-mass}; the two agree at such roots, but at complex
roots $|P(1)|$ can exceed $\prod_j(|\alpha_j|-1)$ ($x^2+\rho^2$).
...
Corollary~\ref{cor:separated}).  For $K_s$ roots in each of several separated
annuli the central index charges a root once for every annulus at or below it,
$\sum_ssK_s\log(T_s/2)-O(S)$ in all when the separation grows with the
number of roots, attained for suitable roots as $\min_sT_s\to\infty$; the cross terms $K_uK_s\log T_s$ of \eqref{eq:order} fail
(Theorem~\ref{thm:annuli},
Corollary~\ref{cor:annuli}, Proposition~\ref{prop:annulisharp}).
...
\eqref{eq:order} applies.  In a sector of half-width $\delta\le\pi/4$ with
$R\ge6$ this
gives mass at least $\frac{K(K-2)}8\log(\frac R2-1)$ for multiples with
leading coefficient of modulus at least $1$ and degree at most
$\pi K/(2E(2\delta))$, where
...
needs $E(2\delta)\le\pi/2$, which forces $\delta<0.033$, so the theorem is
for $\delta\to0$.  At distinct Gaussian-integer roots in the
upper half-plane, where a carry
argument charges degree against mass (Theorem~\ref{thm:gausscharge}), every
integer multiple pays the smaller of this and
$\pi K\log(R/2)/(2E(2\delta))$ (Theorem~\ref{thm:thingauss}), which is of
order $K^2\log R$ when $\delta K\log(e\pi/(2\delta))=O(1)$.  In a sector of
half-width $\delta<\pi/2$ the crossings
are counted against the equidistributed share $\delta/\pi$ of the
large zeros; as $K,R\to\infty$, multiples of degree $O(K)$ with leading coefficient of modulus at least $1$ pay
$(\frac\pi\delta-o(1))K\log R$ at fixed $\delta$ and $(\frac12-o(1))K^2\log R$
if $\delta K\to0$, both sharp (Theorem~\ref{thm:widesector},
Corollaries~\ref{cor:wideconst} and~\ref{cor:thinconst},
Proposition~\ref{prop:widesharp}).  Whether
$\Lambda(F)\ge cK^2\log R$ holds for all integer multiples at distinct Gaussian roots
in the upper half-plane and in a sector of fixed half-width is open.  Two integer imitations of $x^N-\rho^N$ are
...
the lowest coefficient costs $2Km\log\min_j|\alpha_j|$
(Proposition~\ref{prop:gausslow}).
...
$\Lambda(F)\ge K^2/80$ for monic $F$ when $K\ge8$; and for $K=2$ the row
$k=2$ is an infimum at $(c_1^2,c_2^2)=(2,3)$.
\end{enumerate}
\end{corollary}
...
For real roots the mass bound \eqref{eq:order} charges the $i$-th smallest
...
square term is false (Proposition~\ref{prop:noangle}).  This section shows
that the cross terms are false as well, and that the right count charges a
root once for every annulus at or below it: when the separation grows with
the number of roots, every monic multiple has mass at least
...
\emph{central index} at $r$.  In the notation of Theorem~\ref{thm:archnewton},
...
edge with $\log|f_i|=\varphi(i)$, its two vertices included.
...
Let $F\in\mathbb C[x]$ be nonzero of degree $D$, let $n\ge1$ and $u<w$ with
$w-u>2n\log3$, and suppose that $F$ has at least $n$ zeros, counted with
multiplicity, of modulus at least $e^w$.  Then every central index $y$ of $F$
at $e^u$ satisfies $D-y\ge n$.
\end{lemma}

\begin{proof}
We use the tropical roots $\sigma_1>\cdots>\sigma_L$, the vertices $v_s$ and
the edge lengths $\ell_s$ of Theorem~\ref{thm:archnewton}.  Two facts first.
(a) If $u<p$, $y$ is a central index at $e^u$ and $y'$ one at $e^p$, then
$y\le y'$: otherwise $\log|f_y|+yu\ge\log|f_{y'}|+y'u$ and
$\log|f_{y'}|+y'p\ge\log|f_y|+yp$ add up to $(y-y')(u-p)\ge0$, which is false
for $y>y'$ and $u<p$.  (b) Every central index $y$ at $e^u$ satisfies
$D-y\ge\sum_{\sigma_s>u}\ell_s$.  Indeed let $s$ be the largest index with
...
Choose $0<\eta<w-u-2n\log3$ and let $I=[u+\log3+\eta,\,w-\log3]$, an interval
of length greater than $2(n-1)\log3\ge0$.  If some $p\in I$ has
$|p-\sigma_s|>\log3$ for every $s$, then by Theorem~\ref{thm:archnewton},
item~1, $F$ has exactly $y'$ zeros in $|z|<e^p$ and none on $|z|=e^p$, where
$y'$ is the vertex active at $p$.  The $n$ zeros of modulus at least
$e^w>e^p$ lie outside, so $D-y'\ge n$, and $y\le y'$ by (a).  Otherwise the
closed intervals $[\sigma_s-\log3,\sigma_s+\log3]$ that meet $I$ cover it, so
their total length is at least the length of $I$, and there are at least $n$
of them.  Their centres lie in $[u+\eta,w]$, so at least $n$ tropical roots
exceed $u$, and (b) gives $D-y\ge n$.
\end{proof}

The distance cannot be a constant: $(x-t)^K$ has $K$ zeros of modulus $t$, and
at $r=t/c$ its terms are $t^K\binom Kic^{-i}$, so for $c<K$ the term $i=1$
beats the term $i=0$ and the central index is positive.  So
Lemma~\ref{lem:tropcount} needs $e^{w-u}$ at least about $n$; it asks for
$9^n$.
...
$9^{\kappa_a}<T_a/r_a$, and put $k_a=\min\bigl(n_a,\max(\kappa_a,S-a+1)\bigr)$.
...
$\sigma^{(a)}=\sigma_q$ in the numbering of Theorem~\ref{thm:archnewton}.  By
fact~(b) in the proof of Lemma~\ref{lem:tropcount}, a central index at a
...
$D-y_a\ge S-a+1$ by fact~(b) in the proof of Lemma~\ref{lem:tropcount}.  If
$\kappa_a\ge1$, let $n=\min(n_a,\kappa_a)$; Lemma~\ref{lem:tropcount} with
$u=\log r_a$ and $w=\log T_a$ applies, as $w-u=\log(T_a/r_a)>n\log9$ and the
$n_a\ge n$ zeros in $A_{\ge a}$ have modulus at least $T_a$, and gives
$D-y_a\ge n$.  So $D-y_a\ge k_a$ for every $a\ge2$.
...
\begin{enumerate}
\item If moreover $T_a>3\cdot9^{n_a}U_{a-1}$ for $2\le a\le S$, then
...
\begin{proof}
1.  Choose $r_a$ with $3U_{a-1}<r_a<9^{-n_a}T_a$.  Then $r_a<T_a/3$ and
$9^{n_a}<T_a/r_a$, so $k_a=n_a$ and
...
times in all, at the price of a separation that grows exponentially with the
number of roots above.  Item~2 needs only the factor $9$, but past the first
...
gives Corollary~\ref{cor:separated}, item~1, with $\rho_j/2$ in place of
$\rho_j/3$ and an extra factor $\frac12$ in each row.  Item~1 is sharp to
leading order, the coefficient $s$ of each $K_s\log T_s$ being best possible, as $\min_sT_s\to\infty$ (Proposition~\ref{prop:annulisharp}
has $\Lambda=\sum_ssN_s\log t_s$ against the bound
$\sum_ssN_s\log(t_s/6)-S\log2$), and the cross terms $K_uK_s\log T_s$ of the real bound
fail.
...
\]
If moreover $t_1>9$ and $t_a>27\cdot9^{n_a}t_{a-1}$, where
$n_a=N_a+\cdots+N_S$, then Corollary~\ref{cor:annuli}, item~1, applies to $F$
with $A_s$ the zeros of modulus in $[t_s/3,3t_s]$, and gives
$\Lambda(F)\ge\sum_ssN_s\log(t_s/6)-S\log2$.
...
$T_a>3\cdot9^{n_a}U_{a-1}$ (which implies $T_a>9U_{a-1}$), $|A_s|=N_s$,
$f_D=1$, and $|\beta|/2\ge t_s/6$ on $A_s$.  The central index $y_a$ of the
theorem, at $r_a\in(9t_{a-1},t_a/9)$, is the vertex $p_{a-1}$ active there.
...
For the final sentence, item~1 with $S=2$, $N_1=N_2=N$ and $T_s=t_s/3\ge3$
...
So at complex roots in separated annuli the analogue of $\sum_ii\log r_i$ is
$\sum_ssK_s\log T_s$: the cross term between annuli $u<s$ is $K_s\log T_s$,
linear in the number of roots above and independent of the number below.  It
is proved when the separation grows like $9^{n_a}$
(Corollary~\ref{cor:annuli}, item~1), and with ratios in place of moduli at
the fixed separation $9$ (item~2).  Whether item~1 holds at a fixed
separation is open; the only loss is in the second case of the proof of
Lemma~\ref{lem:tropcount}, when tropical roots crowd the whole gap below an
annulus.
...
$E(2\delta)\le\pi/2$, which forces $\delta<0.033$: the theorem is meant for
...
through the Mahler measure.  So for fixed $\delta$ and $K,R\to\infty$
every multiple of degree $O(K)$ pays $(\pi/\delta-o(1))K\log R$, and this
...
degree restriction is removed by Theorem~\ref{thm:gausscharge}
...
multiples at Gaussian roots it is open; the section ends with two limits on
integer imitations of $x^N-\rho^N$ (Lemma~\ref{lem:gaussbinom},
Proposition~\ref{prop:gausslow}).
...
quadratic mass by Corollary~\ref{cor:ray} with
...
(Corollary~\ref{cor:thinconst}), and $\pi$ in $\frac\pi\delta K\log R$ at
...
\paragraph{Integer imitations of $x^N-\rho^N$.}  Corollary~\ref{cor:widegauss}
...
$x^N-\rho^N$, whose mass sits in a single coefficient after a gap of length
$N$.  Its two integer imitations are limited at Gaussian roots: a
...
(Theorem~\ref{thm:transfer}), carries at most $2\deg A$ of them.

For the gap, recall $P(0)=\prod_j\rho_j^2$.  Expand
$1/P(x)=\sum_{l\ge0}u_lx^l$ as a power series at $0$, which is possible since
$P(0)\ne0$, and for $m\ge1$ let $d_m$ be the least positive integer with
$d_mu_l\in\mathbb Z$ for every $l<m$.

\begin{proposition}[The lowest coefficients]\label{prop:gausslow}
In the setting of Section~\ref{sec:gaussian}, let $m\ge1$.
\begin{enumerate}
...
and $G\in\mathbb Z[x]$, then $P(0)^m$ divides $f_e$, and
\[
...
Nor, numerically, can a multiple whose mass sits in a single heavy
coefficient carry many prescribed roots.  If $F=H(x)-v$ with
$H(0)=0$ and $v\in\mathbb Z$, the prescribed roots of $F$ are Gaussian $y$ with
$\operatorname{Im}y>0$ at which $H$ takes the real value $v$.  An exhaustive
search over the $H\in\mathbb Z[y]$ with $H(0)=0$, positive leading coefficient,
coefficients at most $B$ in absolute value and degree at most $s$, and over the
$y$ with $|\operatorname{Re}y|\le Y$ and $1\le\operatorname{Im}y\le Y$, for
$(B,s,Y)=(1,10,30)$, $(2,7,40)$, $(3,5,50)$ and $(5,4,60)$, finds no real value
taken at more than three such $y$, and three only for $H=y^6-2y^4+y^2$, at
$\{\pm2+i,2i\}$, $\{\pm7+4i,8i\}$ and $\{\pm26+15i,30i\}$.  This is numerical
evidence only; whether $\Lambda(F)\ge cK^2\log R$ holds for all integer multiples
at Gaussian roots in a wide sector remains open.


\paragraph{Data availability.}
No datasets were generated or analyzed in this study.  The search of
Section~\ref{sec:widesectors} is \texttt{tools/levelsets.py} of the
accompanying repository, and its small cases are rechecked in
\texttt{tests/test\_complex.py}.
