...
greater than one under an extra sign condition~\cite[p.~333]{dj}, a
condition removed by the $u=0$ clause of~\cite[Theorem~3.2]{coefficient-mass};
the two agree at such roots, but at complex
...
(Lemma~\ref{lem:gaussbinom}; none unless $\rho^{2M}\in\mathbb Z$).  It
...
Lemma~\ref{lem:gaussblocks}; for the rows, take the block of the hypothesis.
...
every multiple of degree $O(K)$ with $|f_D|\ge1$ pays
...
Lemma~\ref{lem:gaussblocks}, of degree $d$; it is
...
coefficient $v=f_e$.
\begin{enumerate}
\item For any decomposition of $F$ as in Lemma~\ref{lem:gaussblocks}, the
...
decomposition of $F$ as in Lemma~\ref{lem:gaussblocks} has exactly one block,
...
$\rho_{\min}=\sqrt{65}>4$.  Whether the number of Gaussian points above the
