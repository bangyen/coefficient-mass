...
$\zeta^m=1$, of real points all of one sign, in particular purely imaginary pairs $\pm ic$, reduce exactly to
...
$\min(K^2,K/\delta)\log R$, with the constants $\frac12$ of $K^2\log R$ (as
$\delta K\to0$) and $\pi$ of $(K/\delta)\log R$ (at fixed $\delta$) attained
in the limit; at
...
in a wide sector is open.  The integer imitations of $x^N-\rho^N$ give no
counterexample: $A((x-t)^N)$ has at most two Gaussian roots in the upper
half-plane per root of $A$ (Lemma~\ref{lem:gaussbinom}), and at odd,
pairwise coprime norms $|\alpha_j|^2$ with
$\gcd(\operatorname{Re}\alpha_j,\operatorname{Im}\alpha_j)=1$ a gap $g$ after
the lowest coefficient costs $2Kg\log\min_j|\alpha_j|$
...
integer imitations of $x^N-\rho^N$ are not cheap (Lemma~\ref{lem:gaussbinom},
Proposition~\ref{prop:gausslow}).
...
$\max_{|t|=\tau}|B^*|\le\max_i|b_i|\sum_k\tau^k=(n+1)\max_i|b_i|$.  So
...
sector and all zeros of $Q$ in $|z|\le R/T$, then $n\le2K$ in the proof.
For $m$ with $\nu_m>2K$ its alternative $n>(\pi/\delta')(K-1-m-\varepsilon')$
fails for $\delta'$ near $\delta$, so the crossings step applies; the largest
such $m$ is at least $(1-\frac{2\delta}\pi)K-e-2$, and whenever this is
nonnegative,
...
as $R\to\infty$, with both constants attained in the limit: $\frac12$ in
$\frac12K^2\log R$, from Theorem~\ref{thm:widesector} with $m=K-o(K)$ as
$\delta K\to0$, when $e=o(K)$ and $\nu_m\ge K^2/2$, against the common ray; and $\pi$ in
$\frac\pi\delta K\log R$, from Corollary~\ref{cor:wideconst} at fixed $\delta$,
against Proposition~\ref{prop:widesharp}.  In a wide sector, with $\delta$
...
$\nu_m=\frac\pi\delta(K-1-m-e)^+$ for this $e$.  In particular, for fixed
...
$N$.  Neither of its two integer imitations is cheap at Gaussian roots: a
polynomial in $(x-t)^N$ has few Gaussian roots, and at pairwise coprime norms
(under the hypotheses of Proposition~\ref{prop:gausslow}, item~2) a long gap
...
$F\in\mathbb Z[x]$ is divisible by $P$ and $F=x^e(f_e+x^gG)$ with $f_e\ne0$,
$g\ge1$ and $G\in\mathbb Z[x]$, then $P(0)^g$ divides $f_e$, and
\[
