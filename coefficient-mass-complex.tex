...
an Archimedean Newton polygon argument.  With $K_s$ roots of modulus at
least $T_s$ in the $s$-th of $S$ annuli, with $T_1>3$ and consecutive annuli separated by
factors $3\cdot9^{K_s+\cdots+K_S}$ growing with the number of roots
(whether a fixed factor suffices is open), monic
multiples have mass at least $\sum_ssK_s\log(T_s/2)-O(S)$, attained to
leading order as $\min_sT_s\to\infty$: a root is charged once per annulus at or below it, and the
...
quadratic mass by Corollary~\ref{cor:ray}), and $\Lambda\ge cK^2\log R$ fails already
