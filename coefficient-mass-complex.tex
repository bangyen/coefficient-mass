...
least $T_s$ in the $s$-th of $S$ sufficiently separated annuli, monic
...
half-width $\delta$, multiples of degree $O(K)$ have mass
...
in a wide sector is open.  Two integer imitations of $x^N-\rho^N$ are
...
degree $O(K)$, as $K,R\to\infty$, both constants are attained: $\frac12$ in
$\frac12K^2\log R$ when $\delta K\to0$, from
Theorem~\ref{thm:widesector} with $T$ fixed and $m=K-1-\lceil\omega+\delta K^2/(2\pi)\rceil$
(then $\omega=\kappa(\delta)d/\log T=o(K)$ as $\kappa(\delta)\to0$, so
$m=K-o(K)$, and $\nu_m\ge K^2/2$), against the common ray; and $\pi$ in
$\frac\pi\delta K\log R$, from Corollary~\ref{cor:wideconst} at fixed $\delta$
as $K,R\to\infty$, against Proposition~\ref{prop:widesharp}.  In a wide
sector, with $\delta$ fixed, the infimum is linear in $K$, and
$\Lambda\ge cK^2\log R$ fails already for the monic real multiple $x^N-\rho^N$.
In between the degree bound matters: $x^N-\rho^N$ has degree about
$\pi K/\delta$, which is not $O(K)$ uniformly in $\delta$.
