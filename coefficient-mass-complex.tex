...
least $T_s$ in the $s$-th of several sufficiently separated annuli, monic
multiples have mass at least $\sum_ssK_s\log(T_s/2)-O(S)$, attained to
leading order: a root is charged once per annulus at or below it, and the
...
number of roots; the cross terms $K_uK_s\log T_s$ of \eqref{eq:order} fail
...
in a wide sector is open.  Its integer imitations of $x^N-\rho^N$ fail: a
polynomial in $(x-t)^N$ has at most two Gaussian roots in the upper
half-plane per root of its outer factor (Lemma~\ref{lem:gaussbinom}), and at
pairwise coprime norms a gap $g$ after the lowest coefficient costs
$2Kg\log R$ (Proposition~\ref{prop:gausslow}).
...
terms, with the constant $1$ attained in the limit
(Corollary~\ref{cor:annuli}, Proposition~\ref{prop:annulisharp}).
...
Let $F\in\mathbb C[x]$ be nonzero of degree $D$, let $n\ge1$ and $u<v$ with
$v-u>2n\log3$, and suppose that $F$ has at least $n$ zeros, counted with
multiplicity, of modulus at least $e^v$.  Then every central index $y$ of $F$
at $e^u$ satisfies $D-y\ge n$.
\end{lemma}

\begin{proof}
We use the tropical roots $\sigma_1>\cdots>\sigma_L$, the vertices $v_s$ and
the multiplicities $\ell_s$ of Theorem~\ref{thm:archnewton}.  Two facts first.
...
$\sigma_s>u$ ($s=0$ if there is none); then $D-v_s=\sum_{t\le s}\ell_t$, and
for $i>v_s$ the edge of slope $-\sigma_s$ gives
$\log|f_i|+iu\le\log|f_{v_s}|+v_su-(\sigma_s-u)(i-v_s)<\log|f_{v_s}|+v_su$,
so $y\le v_s$.

Choose $0<\eta<v-u-2n\log3$ and let $I=[u+\log3+\eta,\,v-\log3]$, an interval
...
$e^v>e^p$ lie outside, so $D-y'\ge n$, and $y\le y'$ by (a).  Otherwise the
...
of them.  Their centres lie in $[u+\eta,v]$, so at least $n$ tropical roots
...
Lemma~\ref{lem:tropcount} needs $e^{v-u}$ at least about $n$; it asks for
$9^n$.
...
$\sigma^{(a)}=\sigma_s$ in the numbering of Theorem~\ref{thm:archnewton}.  By
fact~(b) in the proof of Lemma~\ref{lem:tropcount}, a central index at a
radius below $e^{\sigma_s}$ is at most the lower vertex $v_s$ of the edge of
slope $-\sigma_s$; symmetrically, for $i<v_{s-1}$ the same edge gives
$\log|f_i|+ip<\log|f_{v_{s-1}}|+v_{s-1}p$ when $p>\sigma_s$, so a central
index at a radius above $e^{\sigma_s}$ is at least the upper vertex
$v_{s-1}>v_s$.  Hence $y_a\le v_s<v_{s-1}\le y_{a+1}$ for $a<S$, and
$y_S\le v_s<v_0=D$ for $a=S$.
...
$u=\log r_a$ and $v=\log T_a$ applies, as $v-u=\log(T_a/r_a)>n\log9$ and the
...
constant included, and the cross terms $K_uK_s\log T_s$ of the real bound
fail.
...
$t_{a+1}>9t_a$.  Put $y_a=N_1+\cdots+N_a$ ($y_0=0$), $D=y_S$ and
\[
  F=\sum_{a=0}^Sc_ax^{y_a},\qquad c_a=\prod_{s>a}t_s^{N_s}.
\]
...
with $A_s$ the zeros in $[t_s/3,3t_s]$, and gives
...
roots with moduli in $[T_2,9T_2]$, however large $T_2/T_1$ is, nor for such
roots in conjugate pairs within a fixed angle of the imaginary axis.
\end{proposition}

\begin{proof}
1.  $F$ has the $S+1$ nonzero coefficients $c_a\in\mathbb Z$ at the positions
$y_a$, and $c_S=1$.  The slope from $(y_{a-1},\log c_{a-1})$ to
$(y_a,\log c_a)$ is $-\log t_a$, which decreases strictly with $a$, so these
points are the vertices of the least concave majorant, and the tropical roots
are the $\log t_a$, the edge of $\log t_a$ joining $y_{a-1}$ to $y_a$.  By
Theorem~\ref{thm:archnewton}, item~2, every zero lies in one of the closed
annuli $[t_s/3,3t_s]$, which are disjoint as $t_{s+1}>9t_s$.  For $0\le a\le S$
...
$u_a$ is $y_a$; by Theorem~\ref{thm:archnewton}, item~1, $F$ has exactly
$y_a$ zeros in $|z|<e^{u_a}$, that is, in the first $a$ annuli.  So the
$a$-th annulus holds $y_a-y_{a-1}=N_a$ zeros.  Every $c_a\ge1$, so
...
$f_D=1$, and $|\beta|/2\ge t_s/6$ on $A_s$.
...
multiples, the correct order being $\min(K^2,K/\delta)\log R$.  For integer
multiples at Gaussian roots it is open; the section ends by showing that the
integer imitations of $x^N-\rho^N$ fail (Lemma~\ref{lem:gaussbinom},
...
$N$.  Its two integer imitations fail at Gaussian roots: a polynomial in
$(x-t)^N$ has few Gaussian roots, and at pairwise coprime norms a long gap
after the lowest coefficient is expensive.
...
So $x^N-m$, $m\in\mathbb Z$, carries at most two of the prescribed roots
$\alpha_j$, whatever $N$ and $m$, where Proposition~\ref{prop:widesharp} needs
...
$F=x^e(f_e+x^gG)$ with $f_e\ne0$, $g\ge1$ and $G\in\mathbb Z[x]$ is divisible by
$P$, then $P(0)^g$ divides $f_e$, and
\[
  \Lambda(F)\ \ge\ g\log P(0)\ \ge\ 2Kg\log\rho_{\min}.
\]
...
$F_0=f_e+x^gG$, and $F_0\equiv f_e\pmod{x^g}$, so $d_g=P(0)^g$ divides $f_e$.
Then $\Lambda(F)\ge\log|f_e|\ge g\log P(0)$, and
...
with a gap $g\ge\pi K/\delta$ after its lowest coefficient, pays at least
...
the lowest $m=O(K)$ coefficients of a multiple can carry mass at
most $(K+m-1)\log n$, and no bound that looks only at them gives order
