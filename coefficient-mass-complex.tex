...
as $K\to\infty$, uniformly in $R$; whether they must pay order $K^2\log R$ is
open.
\end{abstract}
...
Corollary~\ref{cor:separated}).  For $K_s$ roots with moduli in $[T_s,U_s]$,
...
$H$ with small nonleading coefficients
(Proposition~\ref{prop:gaussheavy}); an exhaustive search over $H$ of degree
at most $s$ with all coefficients, the leading one included, of modulus at
most $B$, and over Gaussian $y$ with $|\operatorname{Re}y|\le Y$ and
$1\le\operatorname{Im}y\le Y$, for $(B,s,Y)=(1,11,30)$, $(2,8,40)$,
$(2,10,20)$, $(3,8,30)$, finds no level set with more than three such $y$
...
root once for every annulus at or below it: when the moduli of the $K_s$ roots
...
lies in one level set $\{y:H(y)=-v\}$ of such an $H$.  In a finite search, level
sets of polynomials with coefficients at most $3$, the leading one included,
contain at most three Gaussian points of a bounded box.  An exhaustive
...
$(B,s,Y)=(1,11,30)$, $(2,8,40)$, $(2,10,20)$ and $(3,8,30)$, finds no real
value taken at more than three such $y$.  For $H$ of degree below $8$, $H+v$ is a real polynomial of
degree at most $7$, so each level set of $H$ has at most three points above
the axis; only these ranges, with $s\ge8$, are informative.  Three such $y$ occur, in these ranges and in
$(3,6,50)$, $(5,5,60)$ and $(10,4,80)$, only for $H=y^6-2y^4+y^2=(y^3-y)^2$, at $\{\pm2+i,2i\}$,
$\{\pm7+4i,8i\}$ and $\{\pm26+15i,30i\}$, and for $H=y^8+3y^4$, at
$\{\pm1+i,i\}$.  With
Proposition~\ref{prop:gaussheavy}, item~2, applied to $\pm F$, the ranges with $s\ge8$ give: if
