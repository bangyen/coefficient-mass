...
Gaussian-integer roots and fixed $\delta$ the degree restriction is removed
...
$\sum_ssK_s\log T_s$ in all, sharply when the separation grows with the
number of roots and $\min_sT_s\to\infty$; the cross terms $K_uK_s\log T_s$ of \eqref{eq:order} fail
...
Theorem~\ref{thm:thinsector}).  At Gaussian-integer roots, where a carry
...
in a sector of fixed half-width is open.  Two integer imitations of $x^N-\rho^N$ are
...
the number of roots, the mass is $\sum_ssK_s\log T_s$ up to lower order
terms, with the constant $1$ attained as $\min_sT_s\to\infty$
(Corollary~\ref{cor:annuli}, Proposition~\ref{prop:annulisharp}).
...
infimum is therefore linear in $K$, and $\Lambda\ge cK^2\log R$ fails already
