...
$[T_s,U_s]$ for $s=1,\ldots,S$, where $T_1>3$ and $T_s>162U_{s-1}$,
multiples with leading coefficient of modulus at least $1$ have mass at least
$\sum_ssK_s\log(T_s/2)-O(S)$,
...
for integer multiples, which pay $(\frac\pi\delta-o(1))K\log\max(R,\sqrt K)$
as $K\to\infty$, uniformly in $R$.
\end{abstract}
...
$\sum_ssK_s\log(T_s/2)-O(S)$ in all for multiples with $|f_D|\ge1$ at the
fixed separation factor $162$, attained for suitable roots as $\min_sT_s\to\infty$; the cross terms $K_uK_s\log T_s$ of \eqref{eq:order} fail
(Theorem~\ref{thm:annuli},
Corollary~\ref{cor:annuli}, Theorem~\ref{thm:fixedgap},
Proposition~\ref{prop:annulisharp}).  Dividing out the roots counts them
from a central index at a distance linear in their number
...
Proposition~\ref{prop:widesharp}).  At distinct Gaussian roots in the upper
half-plane and fixed $\delta$, integer multiples of every degree pay
$(\frac\pi\delta-o(1))K\log R$ as $K,R\to\infty$
(Corollary~\ref{cor:widegauss}), and, since fewer than $\pi(X+1)^2$ Gaussian
integers have modulus below $X$, $(\frac\pi\delta-o(1))K\log\max(R,\sqrt K)$
as $K\to\infty$, uniformly in $R$ (Corollary~\ref{cor:gausscount}).  Whether
...
(Proposition~\ref{prop:gausslow}).  If $\min_j|\alpha_j|>2$ and an integer
multiple $F$ has $b_2(F)<\min_j|\alpha_j|/2$, all the $\alpha_j$ lie in one level set
$\{y:H(y)=-v\}$ of a polynomial $H$ with small nonleading coefficients
(Proposition~\ref{prop:gaussheavy}); an exhaustive search over $H$ of small
degree with all coefficients small finds, in the ranges searched, no level set
with more than three Gaussian points above the axis (numerical evidence
only).
...
root once for every annulus at or below it: when consecutive annuli are
separated by the factor $162$, every multiple with $|f_D|\ge1$ has mass at least
...
index of $(x-t)^K$ at $t/c$ is about $K/(c+1)$.
...
$Y_S$ below $D$.  So the positions in $\{y_1\}\cup Y_2\cup\cdots\cup Y_S$
...
Theorem~\ref{thm:fixedgap} bounds the mass only: in the second case the
charge $Z_a$ is split over two coefficients, and whether the rows of
...
(Corollary~\ref{cor:widegauss}), and every such multiple pays
...
Over the reals $x^N-\rho^N$ has $b_2=0$.  At Gaussian roots a small second
row confines the multiple to one level set of a polynomial with small
nonleading coefficients.  Call a coefficient $f_i$ of $F$ \emph{heavy} if
$|f_i|\ge\rho_{\min}/2$.
...
coefficient $v=f_e$.
\begin{enumerate}
\item The lowest coefficient of every block of $F$ (Lemma~\ref{lem:gaussblocks})
is heavy and nonleading, so $F$ has at most as many blocks as heavy nonleading
coefficients.
...
2.  At most one nonleading coefficient of $F$ is heavy, so by item~1 $F$ has
exactly one block, $F=x^eB$, and $B(0)=v$ is the heavy one; every other
...
Lemma~\ref{lem:gausscarry} some $i<d$ has
...
lies in one level set $\{y:H(y)=-v\}$ of such an $H$.  Numerically, level
sets of polynomials with small coefficients are small.  An exhaustive
...
$(B,s,Y)=(1,11,30)$, $(2,8,40)$, $(2,10,20)$, $(3,6,50)$, $(3,8,30)$,
$(5,5,60)$ and $(10,4,80)$, finds no real value taken at more than three such
$y$, and three only for $H=y^6-2y^4+y^2=(y^3-y)^2$, at $\{\pm2+i,2i\}$,
$\{\pm7+4i,8i\}$ and $\{\pm26+15i,30i\}$, and for $H=y^8+3y^4$, at
$\{\pm1+i,i\}$.  A real $F$ of degree below $8$ has at most three roots above the
axis, so only the ranges with $s\ge8$ are informative.  With
Proposition~\ref{prop:gaussheavy}, item~2, applied to $\pm F$, they give: if
$K\ge4$, $2B<\rho_{\min}$, every
$\alpha_j$ lies in the box, and every coefficient of $F$ but one has modulus at
most $B$ (the exception is $f_e$, as $|f_e|\ge P(0)>B$), then
$\deg F-e>s$, so $|f_e|\ge(\rho_{\min}/2)^{s+1}$.  This is numerical evidence
only; whether the level sets stay bounded, and whether
