...
| `tests/test_sectors.py` | real crossings next to a sector (Sturm counts), thin and wide sectors, the sharp constant at `x^N - rho^N`, integer imitations of `x^N - rho^N`, heavy coefficients and level sets at Gaussian roots, the three-point level sets from a Pell equation and the circle they lie near, and Gaussian integers in a disc and a half-annulus |
