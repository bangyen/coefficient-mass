...
| `tests/test_complex.py` | multisection transfer, imaginary pairs, first row at Gaussian roots, Newton-polygon dominance and separated moduli, several annuli (with the rows failing at separation `n_a` and the mass bound at separation 2), truncations of `prod (1 - x/beta)^(-1)` bounded below and the second row at separation `n_2 + 1` (linear in `lambda`), every row at separation `n_a + 9` with the row 3 failing when only the top separation grows, both signs, and Gaussian blocks, on seeded integer multiples and exact optima |
